...
Небольшой блок-эксплорер для **собственной цепочки этого мира**. У пузыря UNI свой Anvil, и до
появления сканера монитор ни разу не читал ни одного блока. Открывается кнопкой **SCAN** в панели
управления.

| Эндпоинт | Что отвечает |
|----------|--------------|
| `GET /api/chain/scan?limit=40` | последние транзакции, свежие сверху |
| `GET /api/chain/tx/{hash}` | одна транзакция с квитанцией (статус / газ / логи) |
| `GET /api/chain/address/{addr}` | баланс, nonce, размер кода, последние транзакции |

- **Привязан к миру** — читает `ALIEN_EVM_RPC` своего процесса: монитор UNI сканирует пузырь,
  монитор LIVE — свою цепочку.
- **Read-авторизация**, как у всех read-роутов (открыт при `ALIEN_PUBLIC_READ=1`).
- **Имена, а не догадки** — адреса берутся из `config/deployments/*.json` этого мира и из
  `universe_config.json` пузыря; имена методов — из таблицы 4-байтовых селекторов, собранной по
  настоящим ABI. Неизвестный селектор остаётся hex.
- **Предупреждение о подмене сети** — если RPC отвечает не тем chainId, который заявлен в
  конфигурации, скан и `/api/chain/status` возвращают `chain_mismatch`, а панель показывает баннер.
- **Никогда не отдаёт 500** — любая ошибка это пустой скан с полем `note`: сканер не имеет права
  уронить дашборд.

Зачем это нужно: адрес с `code_size: 0`, в который идёт ровный поток *успешных* вызовов, — это
компонент, работающий с контрактом, которого больше нет: вызов к адресу без кода не откатывается.
Именно так реле лотереи UNI девять дней вызывало `openRound()` в пустоту — и именно так тот же
дрейф поймали второй раз за считаные минуты.
