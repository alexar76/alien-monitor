...
> 🌍 **Languages:** **English** · [Русский](README.ru.md) · [Español](README.es.md) · [Français](README.fr.md) · [中文](README.zh.md)
>
> Terminology follows the [localization glossary](https://github.com/alexar76/aicom/blob/main/docs/localization-glossary.md).

<!-- aicom-readme-badges -->
<p align="center">
  <a href="https://github.com/alexar76/alien-monitor/actions/workflows/ci.yml"><img src="https://raw.githubusercontent.com/alexar76/alien-monitor/refs/heads/main/docs/badges/ci.svg" alt="CI" /></a>
  <a href="https://raw.githubusercontent.com/alexar76/alien-monitor/refs/heads/main/docs/badges/coverage.svg"><img src="https://raw.githubusercontent.com/alexar76/alien-monitor/refs/heads/main/docs/badges/coverage.svg" alt="Test coverage" /></a>
  <a href="https://github.com/alexar76/alien-monitor/blob/main/LICENSE"><img src="https://raw.githubusercontent.com/alexar76/alien-monitor/refs/heads/main/docs/badges/license.svg" alt="License: MIT" /></a>
</p>
<!-- /aicom-readme-badges -->
...
**Maps:** **UNI** [https://monitor-uni.modelmarket.dev/](https://monitor-uni.modelmarket.dev/) (`ALIEN_MODE=universe`) · **LIVE** [https://monitor.modelmarket.dev/](https://monitor.modelmarket.dev/) (`ALIEN_MODE=real`). Split: [uni-and-live.md](https://github.com/alexar76/aicom/blob/main/docs/uni-and-live.md).
...
<p align="center">
  ...
  <br>
  <sub>Live ecosystem graph · node inspector · built-in AI assistant — <a href="https://monitor.modelmarket.dev/"><b>open the live demo →</b></a></sub>
</p>
...
A small block explorer for **the realm's own chain**. The UNI bubble runs its own Anvil, and
until this existed nothing in the monitor had ever read a block. Open it with the **SCAN**
button in the control bar.

| Endpoint | What it answers |
|----------|-----------------|
| `GET /api/chain/scan?limit=40` | recent transactions, newest first |
| `GET /api/chain/tx/{hash}` | one transaction with its receipt (status / gas / logs) |
| `GET /api/chain/address/{addr}` | balance, nonce, code size, recent transactions |

- **Realm-scoped** — reads this process's `ALIEN_EVM_RPC`, so the UNI monitor scans the bubble
  and the LIVE monitor scans its own chain.
- **Read-auth**, like every other read route (open when `ALIEN_PUBLIC_READ=1`).
- **Names, never guesses** — addresses come from `config/deployments/*.json` for this realm plus
  the bubble's `universe_config.json`; method names from a 4-byte table built off the real ABIs.
  An unknown selector stays hex.
- **Chain-identity warning** — when the RPC answers as a different chainId than the configured
  network, the scan and `/api/chain/status` return `chain_mismatch` and the panel shows a banner.
- **Never 500s** — any failure is an empty scan with a `note`; a scanner must not take the
  dashboard down.

Why it earns its place: an address with `code_size: 0` receiving a steady stream of *successful*
calls is a component driving a contract that no longer exists — a call to a code-less address
does not revert. That is how the UNI lottery relayer spent nine days calling `openRound()` into
the void, and how the same drift was caught a second time within minutes.
...
- **i18n (EN / RU / ES / FR / ZH)** — JSON locale files, browser-detected language, switcher in the control bar; AI replies in the selected language. Terminology follows [docs/localization-glossary.md](https://github.com/alexar76/aicom/blob/main/docs/localization-glossary.md), enforced by `frontend/src/__tests__/glossary.test.ts`

### Localization

| Locale | UI strings | README |
|--------|------------|--------|
...
All five carry the same 344 keys; parity, empty strings and glossary terms are held
by tests, so a locale cannot silently fall behind.
...
**Factory catalog:** UNI syncs **`GET /api/products`** from AI-Factory every ~60s and on bootstrap; products appear as **star clusters** near Factory. Failures/timeouts do **not** wipe clusters (see [docs/uni-troubleshooting.md](https://github.com/alexar76/aicom/blob/main/docs/uni-troubleshooting.md) §16).  
...
Troubleshooting: [docs/uni-troubleshooting.md](https://github.com/alexar76/aicom/blob/main/docs/uni-troubleshooting.md).

Public URLs (with nginx): **UNI** https://monitor-uni.modelmarket.dev/ · **LIVE** https://monitor.modelmarket.dev/

Universe compose polls `ALIEN_UNI_HUB_URL` (`https://uni.modelmarket.dev`). LIVE compose uses `ALIEN_PUBLIC_HUB_URL` (`https://modelmarket.dev`). Do not point the UNI process at the live hub.

| Variable | Default (prod compose) |
|----------|------------------------|
| `ALIEN_MODE` | `universe` |
| `ALIEN_UNIVERSE_AUTO_START` | `1` |
| `HUB_URL` | `http://127.0.0.1:9083` (API poll; Docker host) |
| `ALIEN_PUBLIC_HUB_URL` | `https://modelmarket.dev` (operator link on node cards) |
...
- **UNI map:** https://monitor-uni.modelmarket.dev/
- **LIVE map:** https://monitor.modelmarket.dev/
