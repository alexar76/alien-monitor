...
function isHubRole(node: EcoNode): boolean {
  return node.id === 'hub' || node.role === 'hub' || (node.icon === 'hub' && node.id.includes('hub'));
}
...
function HubOrbitBelts({ color, radius }: { color: string; radius: number }) {
  ...
  useFrame(() => {
    ...
    g.current.rotation.z += 0.0015;
  });
  return (
    <group ref={g}>
      {[0, 1, 2].map((i) => (
        <mesh key={i} rotation={[Math.PI / 2.4 + i * 0.35, i * 0.55, i * 0.2]}>
          <torusGeometry args={[radius * (1.55 + i * 0.35), 0.012 + i * 0.004, 8, 48]} />
          <meshBasicMaterial
            color={color}
            transparent
            opacity={0.45 - i * 0.1}
...
function EcoNodeMesh({
  ...
  return (
    <group ref={groupRef} position={[baseX, baseY, baseZ]}>
      ...
      {isHub && !isPrimaryHub && <HubOrbitBelts color={nodeColor} radius={nodeSize} />}
...
function focusDistanceForNode(nodeId: string, group: string): number {
  ...
  if (nodeId === 'competing_hub') return 11;
...
function SceneContent({
  ...
  const secondaryHubs = (state?.nodes ?? []).filter(
    (n) => n.id !== 'hub' && (n.role === 'hub' || n.id === 'competing_hub'),
  );
