...
import BasanosCard from './cards/BasanosCard';
import HestiaCard from './cards/HestiaCard';
import HistorCard from './cards/HistorCard';
import DolosCard from './cards/DolosCard';
...
interface Props {
  ...
  themeColor: string;
  mobile?: boolean;
  /** Open the built-in chain scanner on one address (a declared escrow or wallet). */
  onScanAddress?: (address: string) => void;
}

/** Compact explorer / hex URL for the card footer so it cannot overflow the panel. */
export function truncateDisplayUrl(url: string, max = 36): string {
  const raw = url.trim();
  if (raw.length <= max) return raw;
  try {
    const u = new URL(raw);
    const hex = u.pathname.match(/0x[a-fA-F0-9]{12,}/i)?.[0];
    if (hex) {
      const shortHex = `${hex.slice(0, 8)}…${hex.slice(-5)}`;
      const shown = `${u.host}${u.pathname.replace(hex, shortHex)}`;
      if (shown.length <= max + 8) return shown;
    }
    const rest = `${u.host}${u.pathname}`;
    if (rest.length <= max) return rest;
    return `${rest.slice(0, Math.max(12, max - 10))}…${rest.slice(-8)}`;
  } catch {
    return `${raw.slice(0, Math.max(12, max - 10))}…${raw.slice(-8)}`;
  }
}

function NodeCtaLink({
  href,
  label,
  themeColor,
}: {
  ...
  const rawUrl = label.trim() === href.trim();
  const long = rawUrl && href.length > 36;
  const display = long && !hovered ? truncateDisplayUrl(href) : label;
  ...
  const copy = async (e: MouseEvent) => {
    ...
    try {
      ...
      /* clipboard can be denied; the full URL is still on the link */
    }
  };
  ...
  return (
    ...
    >
      <a
        ...
        target="_blank"
        rel="noreferrer"
        title={long ? href : undefined}
        onClick={(e) => e.stopPropagation()}
        className={`min-w-0 flex-1 text-[11px] font-mono font-bold text-center rounded-lg py-2.5 px-2 ${
          long && hovered ? 'break-all whitespace-normal' : 'truncate'
        }`}
      ...
      {long ? (
        <button
          type="button"
          onClick={copy}
          title={t('nodeDetail.copyUrl', undefined, 'Copy')}
          className="shrink-0 px-2 rounded-lg text-[10px] font-mono font-bold"
          style={{ backgroundColor: themeColor + '1f', border: `1px solid ${themeColor}55`, color: themeColor }}
        >
          {copied ? t('nodeDetail.copied', undefined, 'Copied') : t('nodeDetail.copyUrl', undefined, 'Copy')}
        </button>
      ) : null}
    </div>
  );
...
export default function NodeDetail({ node, onClose, themeColor, mobile = false, onScanAddress }: Props) {
  const { t } = useI18n();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4" onClick={(e) => e.stopPropagation()}>
        {/* `min-w-0` is load-bearing: a flex child will not shrink below its content, so the
            title could never wrap and instead stretched the whole panel sideways — visible on
            any peer with a descriptive name ("CHARON — liquidation risk for the side that
            borrows", "ATLAS — planetary sensor map & composite briefs").

            It WRAPS, and it is not clamped. A two-line clamp was here and looked like the fix
            until a narrow phone showed what it actually does: that title needs three lines at
            ~420px, the third was cut, and the result reads as a name that simply stops
            mid-word — the exact thing the clamp was added to prevent. A hub's name is the one
            string in this panel worth reading in full, the panel already scrolls vertically,
            and a name is a few words, not a paragraph. So let it take the lines it needs. */}
        <div className="flex items-start gap-2 min-w-0 flex-1">
          ...
          />
          <h3
            className="text-sm font-semibold min-w-0 leading-snug"
            style={{
              color: themeColor,
              // `anywhere` and not `break-word`: a peer may advertise itself as one long
              // unbroken token, and that must fold inside the panel rather than widen it.
      ...
      <div className="mb-4">
        ...
        {/* A fixture says so, next to its own status. Without this the row reads
            "NETWORK · ACTIVE" for a hub that is a name from a list. */}
        {node.simulated && (
          <span
            className="inline-block ml-2 px-2 py-0.5 rounded text-[10px] font-mono uppercase"
            style={{
              backgroundColor: '#ffcc3318',
              color: '#ffcc33',
              border: '1px solid #ffcc3344',
            }}
            data-testid="node-simulated-badge"
          >
            {t('nodeDetail.simulated', undefined, 'simulated')}
          </span>
        )}
        {node.simulated && (
          <div
            className="mt-1.5 text-[10px] leading-snug text-white/45"
            data-testid="node-simulated-note"
          >
            {t(
              'nodeDetail.simulatedNote',
              undefined,
              node.simulated_note || 'a fixture of the UNI scenario — nothing answers for it',
            )}
          </div>
        )}
      </div>
      ...
      {/* On chain — what this node declared, and what the chain said about it.
          The declaration comes from its hub's signed well-known; the verdict comes from
          this monitor asking the chain itself. Both are shown, because they are different
          claims: an unverified address is a string somebody published, and rendering it
          the same way as a confirmed contract is how a map starts lying politely. */}
      {node.contracts && node.contracts.entries.length > 0 && (
        <div className="mb-4" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-baseline justify-between gap-2 mb-1.5">
            <div className="text-[10px] font-mono uppercase tracking-wider text-white/40">
              {t('nodeDetail.onchain.title', undefined, 'ON CHAIN')}
            </div>
            <div className="text-[10px] font-mono text-white/35 truncate" title={node.contracts.network || ''}>
              {node.contracts.network || node.contracts.chain || ''}
              {typeof node.contracts.chain_id === 'number' ? ` · ${node.contracts.chain_id}` : ''}
              {node.contracts.simulated
                ? ` · ${t('nodeDetail.onchain.simulated', undefined, 'simulated')}`
                : node.contracts.testnet
                  ? ' · testnet'
                  : ''}
            </div>
          </div>
          <div className="space-y-1.5">
            {node.contracts.entries.map((entry) => (
              <div
                key={entry.address}
                className="px-3 py-2 rounded"
                style={{
                  backgroundColor: themeColor + '0a',
                  border: `1px solid ${entry.verified ? themeColor + '38' : '#ffffff14'}`,
                }}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-mono text-[11px] font-semibold uppercase tracking-wide" style={{ color: entry.verified ? themeColor : 'rgba(255,255,255,0.45)' }}>
                    {entry.role}
                  </span>
                  <span
                    className="font-mono text-[10px] shrink-0"
                    style={{ color: entry.verified ? themeColor : 'rgba(255,255,255,0.35)' }}
                    title={entry.reason || ''}
                  >
                    {entry.verified
                      ? t('nodeDetail.onchain.verified', undefined, 'verified on chain')
                      : t('nodeDetail.onchain.unverified', undefined, 'declared, not verified')}
                  </span>
                </div>
                <div className="font-mono text-[10px] text-white/50 break-all mt-0.5" title={entry.address}>
                  {entry.address}
                </div>
                {entry.reason && (
                  <div className="text-[10px] font-mono text-white/35 mt-0.5">{entry.reason}</div>
                )}
                <div className="flex items-center gap-3 mt-1">
                  {/* This monitor's own scanner: works on the bubble too, where no public
                      explorer exists at all. */}
                  {onScanAddress && (
                    <button
                      type="button"
                      onClick={() => onScanAddress(entry.address)}
                      className="font-mono text-[10px] hover:underline"
                      style={{ color: themeColor }}
                    >
                      {t('nodeDetail.onchain.viewTxs', undefined, 'view transactions →')}
                    </button>
                  )}
                  {entry.explorer && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-mono text-[10px] text-white/45 hover:underline"
                    >
                      {t('nodeDetail.onchain.explorer', undefined, 'explorer ↗')}
                    </a>
                  )}
                </div>
              </div>
      ...
      {node.id === 'basanos' && (
        <BasanosCard node={node} themeColor={themeColor} mobile={mobile} t={t} />
      )}
      {node.id === 'hestia' && (
        <HestiaCard node={node} themeColor={themeColor} mobile={mobile} t={t} />
      )}
      {node.id === 'histor' && (
        <HistorCard node={node} themeColor={themeColor} mobile={mobile} t={t} />
      )}
      {node.id === 'dolos' && (
        <DolosCard node={node} themeColor={themeColor} mobile={mobile} t={t} />
      )}
      ...
      {/* Landing / live / node URL — pinned on mobile so the dock cannot cover the exit. */}
      {(() => {
        // A fixture has no addresses at all. Its own flag decides that here, rather than
        // trusting every producer to have withheld the fields: whatever a simulated node
        // carries, this panel does not print it as somewhere a service answers.
        if (node.simulated) return null;
        ...
        const add = (href: string | undefined, label: string) => {
          ...
          seen.add(url);
          items.push({ href: url, label });
        };
        add(node.links?.landing, t('nodeDetail.openLanding', undefined, 'Open landing →'));
        add(node.links?.live, t('nodeDetail.openLive', undefined, 'Open live →'));
        add(node.url, node.url || '');
        // An address that only resolves inside its own operator's network is shown, but never
        // as a link: a foreign hub declares its parts by compose hostname
        // (`http://provenance-ledger:8812`), and a card that offers that as somewhere to click
        // teaches the reader that its links are worthless.
        const internal = (node.url_internal || '').trim();
        // Where these numbers were actually read from. On a host that runs both realms the
        // polled address and the operator-facing link are different services, and which one
        // a figure came from is the difference between "the bubble's mesh" and "the live
        // one" — so the card states it instead of leaving it to the deploy environment.
        const source = (node.source_url || '').trim();
        if (items.length === 0 && !internal && !source) return null;
        return (
          <div className={`pt-3 border-t shrink-0 space-y-1.5 overflow-x-hidden ${mobile ? 'mt-3' : 'mt-4'}`} style={{ borderColor: themeColor + '22' }}>
            {items.map((item) => (
              <NodeCtaLink key={item.href} href={item.href} label={item.label} themeColor={themeColor} />
            ))}
            {internal && (
              <div
                className="rounded border px-2.5 py-1.5 font-mono text-[10px] leading-snug break-all"
                style={{ borderColor: 'rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.45)' }}
                data-testid="node-internal-address"
                title={internal}
              >
                <span className="mr-1.5 uppercase tracking-wider" style={{ color: '#ffcc33' }}>
                  {t('nodeDetail.internalAddress', undefined, 'internal address')}
                </span>
                {internal}
              </div>
            )}
            {source && (
              <div
                className="rounded border px-2.5 py-1.5 font-mono text-[10px] leading-snug break-all"
                style={{ borderColor: 'rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.45)' }}
                data-testid="node-source-address"
                title={source}
              >
                <span className="mr-1.5 uppercase tracking-wider" style={{ color: themeColor }}>
                  {t('nodeDetail.polledFrom', undefined, 'polled from')}
                </span>
                {source}
                {node.source_shared && (
                  <span className="ml-1.5 uppercase tracking-wider" style={{ color: '#ffcc33' }}>
                    · {t('nodeDetail.sharedWithHost', undefined, 'shared with the host')}
                  </span>
                )}
              </div>
            )}
