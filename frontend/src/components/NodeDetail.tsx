    ...
    >
            ...
            <>
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="text-[10px] font-mono uppercase tracking-wider text-white/40">
                  ...
                  ({sensorQ.trim() ? `${gaiaDevices.length}/${node.gaia_live!.devices!.length}` : node.gaia_live!.devices!.length})
                </div>
              </div>
              <input
                type="search"
      ...
      {node.id === 'atlas' && (
        <div className="mb-4" onClick={(e) => e.stopPropagation()}>
          ...
          {(node.atlas_live?.embed_url || node.links?.embed) && (
            <div
              className="mb-3 rounded overflow-hidden"
              style={{ border: `1px solid ${themeColor}33`, background: '#070b10' }}
            >
              <iframe
                title="ATLAS mini map"
                src={node.atlas_live?.embed_url || node.links?.embed}
