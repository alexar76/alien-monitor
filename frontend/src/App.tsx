...
export interface EcoNode {
  ...
  url?: string;
  /** An advertised address that only resolves inside its own operator's network — shown as
   *  text, never as a link (see backend/node_identity.py). */
  url_internal?: string;
  /** The address this node's numbers were polled from — differs from `url` when one host
   *  serves two realms, which is exactly when the reader needs to know. */
  source_url?: string;
  /** `source_url` is the host's shared service, not one declared for this realm. */
  source_shared?: boolean;
  /** A UNI-scenario FIXTURE: a node the simulation invented, with no service behind it.
   *  Painted with a SIMULATED badge, and never with an address or a metric — see
   *  backend/universe_hub_spawner.py, which used to hand these both. */
  simulated?: boolean;
  /** Why this node is a fixture, in one clause. Localized text wins over it. */
  simulated_note?: string;
  detail?: { first_seen?: string; discoverer?: string; note?: string };
  /** What this node says it has ON CHAIN — its escrow, its settlement wallet, the
   *  settlement token — as published in its hub's signed well-known and re-exported by
   *  whoever knows it. `verified` on an entry means THIS monitor asked the chain and the
   *  address holds code (a contract) or has transacted (a wallet); false means it does
   *  not, or that it lives on a chain this monitor cannot read. Never rendered as
   *  confirmed without that flag — an address in a monospace font reads as a fact. */
  contracts?: {
    chain?: string;
    chain_id?: number;
    network?: string;
  ...
  /**
   * What the hearth is actually hosting — public roster only. An empty list is
   * idle, not an empty market; agents appear only after an explicit deploy.
   */
  /** HISTOR, the MCP transparency log: counts keep the window the service publishes them under. */
  histor_live?: {
    version?: string | null;
    store?: string | null;
    ...
    registry_servers?: number | null;
    registry_endpoints?: number | null;
    attempted?: number | null;
    answered_ok?: number | null;
    auth_required?: number | null;
    pinned?: number | null;
    block_tier?: number | null;
    changes?: { last24h?: number | null; last7d?: number | null; last30d?: number | null };
    not?: string[];
  };
  hestia_live?: {
    console_url?: string;
...
export interface EcosystemState {
  ...
  summary: {
    ...
    /** Why this realm's simulated economy is not transacting, if it is not. */
    economy_stalled?: string;
    buyer_rounds?: number;
    buyer_purchases_total?: number;
  };
  scenario?: {
    phase: string;
...
export default function App() {
  ...
  // Which address the scanner should open on. Set when a node card's declared escrow or
  // wallet is clicked; cleared when the scanner closes, so the topbar button still opens
  // on the block list.
  ...
  // A `?node=` deep link is applied ONCE, as soon as that node exists. The graph is re-parsed
  // on every WebSocket tick; without this guard the panel reopened on every tick and a visitor
  // arriving from the landing could never leave it.
  const deepLinkApplied = useRef(false);
  useEffect(() => {
    if (deepLinkApplied.current || !state?.nodes?.length) return;
    ...
    if (!graphState?.nodes?.some((n) => n.id === nodeId)) return; // not drawn yet; next tick
    deepLinkApplied.current = true;
    focusNodeById(nodeId);
    params.delete('node');
    const rest = params.toString();
    window.history.replaceState(null, '', `${window.location.pathname}${rest ? `?${rest}` : ''}${window.location.hash}`);
  }, [state?.nodes, graphState?.nodes, focusNodeById]);
  ...
  // The scanner is a full right-hand panel; on desktop it shares that anchor with AI and
  // Reputation, so opening it closes them. On mobile it is its own fixed sheet.
  ...
  return (
    <div className="relative w-full min-h-[100dvh] h-[100dvh] bg-[#0a0a0f] overflow-hidden">
      ...
      {/* A realm whose buyer cannot buy has no economy, and that is a headline, not a log
          line. The UNI bubble spent ten days with its simulated buyer refused at
          channel/open every round — visible only as one 400 in the container log. */}
      {state?.summary?.economy_stalled && (
        <div
          className="absolute top-2 left-1/2 z-30 max-w-[92vw] -translate-x-1/2 rounded border border-red-500/40 bg-red-950/80 px-3 py-1.5 text-center text-[11px] font-mono text-red-200/90 md:top-3 md:text-xs"
          role="status"
          title={state.summary.economy_stalled}
        >
          {t('monitor.economyStalled')} · {state.summary.economy_stalled.slice(0, 120)}
        </div>
      )}
      ...
      {selectedNode && (!isMobile || mobileSheet === 'node') && (
        selectedNode.id === 'argus' || selectedNode.group === 'argus' ? (
          ...
          <NodeDetail
            ...
            themeColor={themeColor}
            mobile={isMobile}
            onScanAddress={(address) => { setScanAddress(address); setShowScanner(true); }}
          />
        )
      )}
      ...
      {/* Chain scanner — the realm's own chain, block by block (Etherscan-style) */}
      {showScanner && (
        <ChainScanner
          themeColor={themeColor}
          realm={paintedMode ?? 'universe'}
          onClose={() => { setShowScanner(false); setScanAddress(null); }}
          mobile={isMobile}
          initialAddress={scanAddress ?? undefined}
        />
      )}
