    ...
    for (const count of [2, 5, 8, 17, 40]) {
      ...
      for (let i = 0; i < slots.length; i += 1) {
        for (let j = i + 1; j < slots.length; j += 1) {
          // A hub's planets are drawn with a hub's corona, so they need a hub child's gap,
          // not a bare planet's — see HUB_CHILD_MIN_GAP.
          expect(dist(slots[i], slots[j])).toBeGreaterThanOrEqual(HUB_CHILD_MIN_GAP);
        }
      }
    }
  });
...
describe('constellations resolve as you approach', () => {
  const map: EcoNode[] = [
    ...
    // Observed by a hub, approved by nobody — the shelf around the federation, not the
    // federation itself, so this is the one that waits to be asked for.
    node({ id: 'near-pending', label: 'p', group: 'pending_hub_node', hop: 2, parent_id: 'near' }),
    node({ id: 'far-pending', label: 'q', group: 'pending_hub_node', hop: 2, parent_id: 'far' }),
  ];

  it('draws only the pending constellation of a hub that is open', () => {
    const drawn = visibleNodes(map, new Set(['near'])).map((n) => n.id);
    expect(drawn).toContain('near-pending');
    expect(drawn).not.toContain('far-pending');
  });

  it('keeps a federated node with its own system, whether or not its hub is open', () => {
    // Lazy-revealing these meant a hub we actually federate with appeared as a lone dot
    // with no constellation — the one thing this map exists to show.
    const drawn = visibleNodes(map, new Set()).map((n) => n.id);
    expect(drawn).toEqual(['hub', 'near', 'far', 'near-a', 'far-a', 'orphan']);
    expect(drawn).not.toContain('near-pending');
  });
...
describe('cameraBounds — the map must fit inside the clip plane', () => {
  /** The far side of the map, seen from the furthest the orbit allows. */
  const deepest = (b: { radius: number; maxDistance: number }) => b.maxDistance + b.radius;

  it('covers the whole map at full zoom-out, at every size', () => {
    // 48.2 is the live case that broke: `signal_hunt` on monitor.modelmarket.dev. The
    // camera could pull back to 77.1, the far edge sat at 125.3, and the plane was 120 —
    // so the top node vanished and the links crossing it were cut mid-line.
    for (const radius of [0, 5, 30, 48.2, 60, 126, 400, 2000]) {
      const b = cameraBounds([{ position: { x: radius, y: 0, z: 0 } }]);
      expect(b.far).toBeGreaterThan(deepest(b));
    }
  });

  it('measures from the furthest node, not the first', () => {
    const b = cameraBounds([
      { position: { x: 1, y: 0, z: 0 } },
      { position: { x: 0, y: 0, z: 48.2 } },
      { position: null },
    ]);
    expect(b.radius).toBeCloseTo(48.2, 3);
    expect(b.far).toBeGreaterThan(deepest(b));
  });

  it('keeps a floor for a small map, and grows monotonically', () => {
    const empty = cameraBounds([]);
    expect(empty.maxDistance).toBe(52);
    expect(empty.far).toBe(120);
    let previous = empty.far;
    for (const radius of [40, 80, 160, 320]) {
      const next = cameraBounds([{ position: { x: 0, y: radius, z: 0 } }]).far;
  ...
  it('still answers the old question the old way', () => {
    // `cameraRange` is the orbit limit and nothing else — it must not start returning far.
    expect(cameraRange([])).toBe(cameraBounds([]).maxDistance);
    const nodes = [{ position: { x: 0, y: 0, z: 90 } }];
    expect(cameraRange(nodes)).toBe(cameraBounds(nodes).maxDistance);
  });
});
