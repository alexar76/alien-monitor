...
describe('resolveNodeScene', () => {
  ...
  it('resolves the security pair themis / basanos from the registry', () => {
    expect(resolveNodeScene(node({ id: 'themis', group: 'security' }))?.kind).toBe('themis');
    expect(resolveNodeScene(node({ id: 'basanos', group: 'security' }))?.kind).toBe('basanos');
  });
