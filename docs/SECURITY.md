...
**F-1: AI endpoint was unauthenticated** — Severity: Medium (cost)
`/api/ai/ask` may be public when `ALIEN_PUBLIC_READ=1` (demo) or gated by Bearer `ALIEN_API_TOKEN`. Unauthenticated access primarily risks third-party LLM cost, not tool side effects (read-only assistant).
- **Mitigation (shipped):** AEGIS-calibrated prompt firewall hard-rejects CRITICAL / layered STRONG injection before any provider call; user text and monitor snapshots are wrapped as untrusted data.
- **Still recommended:** Rate limit + token for public deployments.
- **Status:** Injection path closed; cost/auth remain deployment controls.
