...
| Alien Monitor **UNI** | [monitor.modelmarket.dev](https://monitor-uni.modelmarket.dev/) | 3D graph of the bubble · `ALIEN_MODE=universe` (factory `/monitor/` redirects here) |
...
| Pulse (ACEX) | [pulse.modelmarket.dev](https://pulse.modelmarket.dev/) · [factory /pulse/](https://magic-ai-factory.com/pulse/) | Capital markets UI |
...
**Signatures are hybrid since 2026-09-06.** Every hub signs its `.well-known` and manifest with
Ed25519 **and** ML-DSA-65 (FIPS 204) over the same canonical string; the `pq_*` fields are additive,
so a verifier that never heard of ML-DSA reads `algorithm`/`value` and is unaffected. Each hub pins
a peer's post-quantum key on FIRST SIGHT and refuses a later withdrawal or substitution — a
per-peer ratchet, deliberately not a global requirement, so joining the federation still costs a
newcomer nothing. Full migration state, the three phases and why the order is forced live in
`docs/pqc-migration.md` (with `.ru` / `.es` / `.fr` / `.zh` beside it) — referenced as a PATH and
not a link on purpose: the mirror absolutises relative links into github.com URLs, and that
document is not mirrored yet, so a link here would hand a reader a 404. It becomes a link on the
