...
@ttl_cached(env_var="ALIEN_ARGUS_CACHE_TTL")
...
def apply_argus_to_nodes(
    ...
    *,
    ...
    if not health or health.get("status") != "ok":
        ...
        # Fleet may still be heartbeating even when /health is down — update
        # ARGUS · N label/metrics, but do not flip explicit offline/error.
        try:
            from argus_roster import apply_roster_to_node

            apply_roster_to_node(argus, promote_status=False)
        except Exception:
            pass
        return
    ...
    # One ball, N agents — Products · N pattern (searchable roster in the panel).
    try:
        from argus_roster import apply_roster_to_node

        apply_roster_to_node(argus)
    except Exception:
        pass
...
def apply_argus_graph(nodes: list[dict], *, mode: str = "real") -> None:
    ...
    if argus is not None:
        ...
        # Roster count even when /health is down (fleet still heartbeating).
        try:
            from argus_roster import apply_roster_to_node, seed_demo_instances, counts

            if mode == "test" and counts()["total"] == 0:
                seed_demo_instances()
            # Never promote offline/error → active just because the roster has demos
            # (CI ALIEN_MODE=test seeds instances; health remaining None must stay offline).
            apply_roster_to_node(
                argus, promote_status=argus.get("status") == "active"
            )
        except Exception:
            pass
