...
#
# Single source of truth = config/deployments/base-mainnet.json (the committed
# registry, mirrored by ARGUS networks.ts and verified against the journal by a
# test). We LOAD from that file so docs↔code can't diverge; the inline dict below
# is a bundled fallback for standalone installs where the repo file isn't shipped.
_BASE_ADDRESSES_FALLBACK: dict[str, str] = {
...
def _load_deployment_contracts(network: str, fallback: dict[str, str]) -> dict[str, str]:
    """Load the committed address registry for ``network``; fall back to ``fallback``.

    Search order: AIFACTORY_DEPLOYMENTS_DIR/<network>-mainnet.json, then walk up
    from this module to find config/deployments/<network>-mainnet.json (monorepo).
    Standalone installs without the file just use the bundled fallback — never crash.
    """
    candidates: list[str] = []
...
_BASE_ADDRESSES: dict[str, str] = _load_deployment_contracts("base", _BASE_ADDRESSES_FALLBACK)
