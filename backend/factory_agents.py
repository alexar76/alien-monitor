...
def factory_agents_node_spec(*, mode: str = "real") -> dict[str, Any]:
    ...
    return {
        "id": "factory_agents",
        # Short map label — roster size lives in metrics + the click card, never here.
        "label": "Agents",
        ...
        # idle (not offline): the roster ball must stay visible even before the first poll.
        "status": "idle",
...
def apply_factory_agents_graph(nodes: list[dict[str, Any]], *, mode: str = "real") -> None:
    ...
    if node is None:
        return
    # Always keep the map label stable (no Agents · N flicker).
    node["label"] = "Agents"
    node["position"] = node_position("factory_agents")
    doc = fetch_agents_sync()
    if not doc:
        node["factory_agents_live"] = {"stale": True, "agents": [], "summary": {}}
        # Stay visible (greyed offline hid the ball among the swarm).
        if node.get("status") in (None, "", "offline", "unknown"):
            node["status"] = "idle"
        return
    ...
    # Never offline the roster ball — empty registry is idle; live agents → active.
    node["status"] = "active" if live else "idle"
