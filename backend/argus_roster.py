...
One graph ball (``argus``) shows ``ARGUS · N``. Click opens a searchable,
...
def apply_roster_to_node(node: dict[str, Any], *, promote_status: bool = True) -> None:
    """Update the singleton ARGUS ball label + metrics from roster counts."""
    c = counts()
    active = c["active"]
    total = c["total"]
    # Prefer live connected count; fall back to total ever-seen in window.
    n = active if active > 0 else total
    node["label"] = f"ARGUS · {n}" if n > 0 else "ARGUS-3"
