...
Live stack runs on the **oracle host** (oracles.modelmarket.dev).
...
def _remap_discovered_id(
    raw: str | None,
    by_id: dict[str, dict[str, Any]],
    skipped: set[str],
) -> str | None:
    """Map a discovery id onto the node the LIVE graph actually draws."""
    from canonical_peers import canonical_node_id_for_peer

    if not raw:
        return None
    nid = str(raw)
    canon = canonical_node_id_for_peer({"id": nid, "label": nid})
    if canon and canon in by_id:
        return canon
    if nid in skipped:
        return canon if canon in by_id else (nid if nid in by_id else None)
    return nid


#: Groups that orbit a peer hub on its child shell. One list, because the layout pass and
#: the collection pass disagreeing about it is the bug this constant exists to prevent.
_HUB_CHILD_GROUPS = ("peer_hub_provider", "peer_hub_node")
...
def merge_discovered_peers(
    ...
    """Fold hub discovery into the graph — family/first-party peers enrich, never duplicate."""
    by_id = {n["id"]: n for n in nodes if n.get("id")}
    existing = set(by_id)
    link_keys = {(l.get("source"), l.get("target")) for l in links}
    ...
    # What each discovery node became. The merge used to keep this only in local variables,
    # so a node that was folded, skipped or silently dropped left no trace anywhere — and the
    # only symptom was a hub missing from a picture. Returned now so `federation_audit` can
    # name the gate instead of a person having to guess it.
    outcome: dict[str, str] = {}
    skipped: set[str] = set()
    new_moons: list[dict[str, Any]] = []
    for dn in disc.get("nodes", []):
        if not isinstance(dn, dict):
            continue
        fam_id = canonical_node_id_for_peer(dn)
        nid = dn.get("id")
        if fam_id and fam_id in by_id:
            ...
            # The on-chain declaration, too. This fold is how a DISCOVERED hub becomes the
            # SEEDED node that already stands for it — `competing_hub` and `signal_hunt_hub`
            # on the live map are seeded suns hydrated by discovery — and it copied numeric
            # metrics only. So the escrow a foreign hub publishes, which discovery had just
            # read and verified against the chain, was dropped precisely for the hubs a
            # viewer is most likely to open. Verified evidence is not a number, and losing
            # it here is invisible: the node keeps working, it just never shows an address.
            if dn.get("contracts"):
                target["contracts"] = dn["contracts"]
            if nid:
                skipped.add(str(nid)); outcome[str(nid)] = "fold:%s" % fam_id
            continue
        if fam_id:
            if nid:
                skipped.add(str(nid)); outcome[str(nid)] = "fold_absent:%s" % fam_id
            continue
        if not nid or nid in existing:
            if nid:
                outcome[str(nid)] = "collision"
            continue
        parent = str(dn.get("parent_id") or "")
        if parent:
            folded_parent = _remap_discovered_id(parent, by_id, skipped)
            if folded_parent:
                dn["parent_id"] = folded_parent
        nodes.append(dn)
        by_id[nid] = dn
        existing.add(nid)
        outcome[str(nid)] = "added"
        if dn.get("group") in _HUB_CHILD_GROUPS:
            new_moons.append(dn)

    # Re-place a hub's constellation around wherever that hub actually ended up — a fold can
    # move it. ALL of its children, and every child it already has, not only the ones this
    # merge added: this pass used to collect `peer_hub_node` alone, so a hub's providers
    # (placed by discovery counting every sibling) and its peers (re-placed here counting
    # only themselves) both took slot 0 of the same shell. Measured on the attested map
    # 2026-09-07: Signal Hunt Hub had one provider and three peers, THEMIS was handed the
    # provider's slot, and the spacing pass nudged the provider 0.68 aside — leaving the two
    # of them 0.971 apart, which is what "THEMIS is glued to Signal Hunt" looks like.
    #
    # The primary hub is excluded on purpose: its own providers are its local star system on
    # a 12-unit orbit (`primary_hub_child_position`), not moons on a 2.2-unit shell.
    parents = {str(child.get("parent_id") or "") for child in new_moons} - {"", "hub"}
    for pid in sorted(parents):
        parent = by_id.get(pid)
        if not parent or not isinstance(parent.get("position"), dict):
            continue
        kids = [
            n for n in nodes
            if str(n.get("parent_id") or "") == pid and n.get("group") in _HUB_CHILD_GROUPS
        ]
        # Providers first, then peers, each alphabetically — the order discovery unpacks
        # them in, so a node does not change orbit when an unrelated peer appears.
        kids.sort(key=lambda n: (
            0 if n.get("group") == "peer_hub_provider" else 1, str(n.get("id") or "")
        ))
    ...
    for link in disc.get("links", []):
        if not isinstance(link, dict):
            continue
        src = _remap_discovered_id(link.get("source"), by_id, skipped)
        tgt = _remap_discovered_id(link.get("target"), by_id, skipped)
