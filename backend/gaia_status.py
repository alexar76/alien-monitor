...
OGC SensorThings, NOAA tide/NDBC, USGS river/quake, Open-Meteo, UK grid,
NASA FIRMS fire, Safecast radiation, CyberNews GNSS, …).
"""
...
# Panel list budget (Alien Monitor GaiaCard). Override with
# ALIEN_GAIA_DEVICE_LIST_LIMIT — 0 or negative = no cap (send full fleet).
# Default 0 (unlimited): Monitor is not the map of record; ATLAS is. A soft
# historical default of 96 caused new LIVE SKUs to vanish behind the OM mesh.
def _device_list_limit() -> int | None:
    raw = (os.environ.get("ALIEN_GAIA_DEVICE_LIST_LIMIT") or "").strip()
    if raw == "":
...
# Singleton / entry-product LIVE relays — always pin to the front of the panel
# so a large OM city mesh cannot push them past a configured list budget.
_PRIORITY_LIVE_IDS = frozenset({
    "firms-fire-01",
    "safecast-01",
    "cybernews-jam-01",
    "usgs-quake-01",
    ...
    "feeder-adsb-01",
    "feeder-ais-01",
    # P5 commercially-clear relays (singleton / mesh parents)
    "metar-kjfk-01",
    "pegel-bonn-01",
    "fintraffic-road-01",
    "fintraffic-rail-01",
...
def _pick_devices_for_panel(
    devices: list[dict[str, Any]], *, limit: int | None = None
) -> list[dict[str, Any]]:
    """LIVE first (priority SKUs ahead of mesh), then SIM. ``limit=None`` = all."""
    if limit is None:
        limit = _device_list_limit()
    live = [d for d in devices if d.get("live")]
    sim = [d for d in devices if not d.get("live")]
    priority = [d for d in live if d.get("id") in _PRIORITY_LIVE_IDS]
    rest = [d for d in live if d.get("id") not in _PRIORITY_LIVE_IDS]
    ordered = priority + rest + sim
    if limit is None:
        return ordered
...
@ttl_cached(env_var="ALIEN_GAIA_CACHE_TTL")
