...
#: First pause after the buyer gives up on a round, doubling per further refusal up to
#: the cap. The hub's own limit is per hour, so the cap is well inside one window.
_OPEN_BACKOFF_S = 60.0
_MAX_OPEN_BACKOFF_S = 900.0
...
class ExternalAIBuyer:
    ...
    def __init__(self, hub_url: str = "http://127.0.0.1:9083"):
        ...
        self._unspent: dict[str, Any] = {}
        self._opened_deposit_usd = 0.0
        self._settle_w3 = None
        self._settle_accounts: list[str] = []
    ...
    def execute_round(self, vu: VirtualUniverse) -> dict:
        ...
        try:
            ...
            if self._open_retry_at and time.time() < self._open_retry_at:
                # Backing off. Every refused open still counts against the hub's
                # "max 20 channel opens per hour per wallet", so a buyer that keeps
                # asking spends its whole quota on refusals and the hub's answer
                # changes from the real reason to "rate limit exceeded" — which is how
                # a wrong-chain deposit came to look like a throttling problem. Wait,
                # and the next attempt gets the true error back.
                self.rounds_completed += 1
                self._last_round_note = (
                    f"holding off {int(self._open_retry_at - time.time())}s before "
                    f"asking for another channel"
                )
                return {
                    "purchases": 0,
                    "events": [],
            ...
            if not channel_id:
                ...
                self.rounds_completed += 1
                if self._open_failures >= _MAX_OPEN_FAILURES:
                    # Doubling, capped: a misconfigured realm stops hammering within a
                    # few rounds, and one that is merely having a bad minute still
                    # recovers on its own without an operator.
                    wait = min(
                        _MAX_OPEN_BACKOFF_S,
                        _OPEN_BACKOFF_S * 2 ** (self._open_failures - _MAX_OPEN_FAILURES),
                    )
                    self._open_retry_at = time.time() + wait
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "unspent_deposit_usd": round(float(self._unspent.get("usd") or 0.0), 6),
            "halted": self._halt_reason,
        }
    ...
    def _settlement_chain(self, vu: Any) -> tuple[Any, str, list[str], str]:
        """(w3, token, unlocked accounts, reason) for the chain THIS HUB verifies on.

        The realm's own chain is the default and the only one most deployments have. The
        bubble is not one of them: `my-vps` runs the monitor's universe Anvil on
        127.0.0.1:8545 and the hub's bubble Anvil on 172.17.0.1:8546, and the hub reads
        deposits from the second one (`AIMARKET_RPC_BASE`). Both answer chain id 31337 and
        both carry the same deterministic addresses from the public test mnemonic — so a
        deposit mined on one and presented to the other matches in every visible field and
        exists on neither ledger the other can see. The hub answered "transaction not found
        or not yet mined", correctly, on every round for as long as the realm has been up:
        the universe chain was at block 553 while the bubble chain, where every one of
        those dollars was supposed to move, was still at block 7.

        So the RPC the deposit is made on is named separately from the RPC the map reads.
        Unset, nothing changes and the realm's own chain is used.
        """
        rpc = (os.environ.get("ALIEN_UNIVERSE_BUYER_RPC") or "").strip().rstrip("/")
        realm_token = str(getattr(vu, "evm_usdt_address", "") or "")
        if not rpc:
            return getattr(vu, "_w3", None), realm_token, list(getattr(vu, "_eth_accounts", None) or []), ""
        token = (os.environ.get("ALIEN_UNIVERSE_BUYER_TOKEN") or realm_token).strip()
        if self._settle_w3 is None:
            try:
                from web3 import Web3

                self._settle_w3 = Web3(Web3.HTTPProvider(rpc, request_kwargs={"timeout": 10}))
                self._settle_accounts = [str(a) for a in self._settle_w3.eth.accounts]
            except Exception as exc:
                self._settle_w3 = None
                return None, token, [], f"settlement chain {rpc} is unreachable ({type(exc).__name__}: {exc})"
        return self._settle_w3, token, self._settle_accounts, ""
    ...
    def _deposit_onchain(self, vu: Any, budget: float, item: dict | None = None) -> tuple[str, str]:
        ...
        """
        w3, token_address, accounts, why = self._settlement_chain(vu)
        ...
        unlocked = {str(a).lower() for a in accounts}
        ...
        if balance < units:
            # Whichever unlocked account actually HOLDS the tokens, not accounts[0] by
            # convention. On the bubble chain `deploy_uni_realm.py` splits the supply
            # between the hub's wallet and the buyer's and leaves accounts[0] with
            # nothing, so the assumed supplier would have reverted the moment the
            # buyer's own float ran out — years of rounds away, and silent when it came.
            top_up = units * 20
            supplier = ""
            for candidate in accounts:
                address = w3.to_checksum_address(candidate)
                if address.lower() == buyer.lower():
                    continue
                if int(token.functions.balanceOf(address).call()) >= top_up:
                    supplier = address
                    break
            if not supplier:
                return "", (
                    f"no unlocked account on this realm's chain holds "
                    f"{top_up / 10 ** decimals:.2f} tokens to top the buyer up with"
                )
    ...
    def _open_channel(self, budget: float, vu: Any = None, item: dict | None = None) -> str | None:
        # A refused open does NOT un-send the money. The deposit is an on-chain transfer
        # made BEFORE the request, and the hub's own limiter — 20 opens per hour per
        # wallet, checked before it verifies anything — refuses plenty of requests whose
        # deposit was perfectly good. Paying again on the next round would strand another
        # one: on the first pass after the chain fix the buyer moved $96.60 to the hub
        # and got a 400 back, which is the hub behaving correctly and the client not.
        #
        # So an unspent deposit is kept and re-presented until some open accepts it. The
        # hub verifies the same transfer either way; nothing here is spent twice.
        if self._unspent:
            tx_hash = self._unspent["tx_hash"]
            payer = self._unspent["payer"]
            budget = float(self._unspent["usd"])
            print(f"[Buyer] re-presenting the unspent ${budget:.2f} deposit {tx_hash[:12]}")
        else:
            ...
            if not tx_hash:
                ...
                return None
            self._unspent = {"tx_hash": tx_hash, "payer": payer, "usd": float(budget)}
        self._opened_deposit_usd = float(budget)
            ...
            with httpx.Client(timeout=15.0) as client:
                for attempt in (1, 2):
                    ...
                    if r.status_code == 200:
                        ...
                        self._unspent = {}          # this deposit is now a channel
    ...
    def _channel_for(
        ...
        if channel_id:
            ...
            # What the hub credited, which is the deposit it verified — not the budget
            # this round happened to draw. A re-presented deposit carries its own amount.
            self._channel_balance = self._opened_deposit_usd or budget
        return channel_id
