...
#: A hub's own peers and providers are drawn by the SAME scene code as a hub (the frontend's
#: `isHubRole` covers all three groups): a body of up to 0.56 radius, a corona at 1.7x, and a
#: label slung underneath it. Calling them 0.9 planets let two of them sit 0.971 apart and
#: pass this check — which is how THEMIS came out glued to Signal Hunt on the attested map
#: (measured 2026-09-07). Two bodies plus room to read a label through is 1.8.
HUB_CHILD_SEPARATION = 1.8

#: Groups the frontend draws with a hub's corona but that are somebody else's planets.
HUB_CHILD_GROUPS = frozenset({"peer_hub_provider", "peer_hub_node"})
...
def clearance_for(node: dict[str, Any]) -> float:
    ...
    if str(node.get("group") or "") in HUB_CHILD_GROUPS:
        return HUB_CHILD_SEPARATION
    if str(node.get("role") or "") == "hub" or str(node.get("group") or "") in SUN_GROUPS:
        ...
        return SUN_SEPARATION if int(node.get("hop") or 0) <= 1 else HUB_CHILD_SEPARATION
...
def _priority(node: dict[str, Any]) -> tuple[int, str]:
    ...
    Anchors, then suns, then a sun's own constellation, then everything else — alphabetically
    by id within a tier. Alphabetical rather than "whatever order the graph was built in"
    because that order changes with what is live, and a map that rearranges itself when an
    unrelated service restarts is unreadable.

    The tiers are ordered by how much room the node claims: letting a 0.9-radius planet take
    a spot first only to displace a 4.6-radius hub around it is how the crowding came back.
    Hub children are a tier of their own rather than sharing the sun tier — they need 1.8, but
    `signal-hunt-hub`'s child sorts before `signal-hunt-hub` itself, so sharing the tier would
    let a moon place first and push its own sun out from under it.
    """
    group = str(node.get("group") or "")
    if group in ANCHOR_GROUPS:
        return (0, _key(node))
    clearance = clearance_for(node)
    if clearance >= SUN_SEPARATION:
        return (1, _key(node))
