...
class EcosystemEntity:
    ...
    def __init__(
        ...
        *,
        ...
        #: The address this entity's numbers were actually polled from. Separate from
        #: ``url`` (which is the operator-facing link) because on a shared host the two
        #: differ, and that difference is the whole question of whose figures these are.
        self.source_url: Optional[str] = None
        #: True when ``source_url`` is the host's shared service rather than one declared
        #: for this realm — the figures are real, they are just not only this realm's.
        self.source_shared: bool = False
        #: True when this entity is a FIXTURE the UNI scenario invented — a name from a
        #: list, not a component anything polled. Nothing answers for it, so its card must
        #: never offer an address or a figure: an address in a monospace font reads as a
        #: service, and a metric reads as a measurement.
        self.simulated: bool = False
        #: Why it is a fixture, in one clause. The frontend prefers its own localized text
        #: and falls back to this.
        self.simulated_note: Optional[str] = None
    ...
    def to_node(self) -> dict:
        ...
        if self.source_url:
            node["source_url"] = self.source_url
            if self.source_shared:
                node["source_shared"] = True
        if self.simulated:
            node["simulated"] = True
            if self.simulated_note:
                node["simulated_note"] = self.simulated_note
        return node
...
def foundry_anvil_tmp_dir() -> Path:
    """Where Foundry writes dump-state trees. Not the demo ``--state`` directory."""
    raw = (os.environ.get("ALIEN_FOUNDRY_ANVIL_TMP_DIR") or "").strip()
    if raw:
        return Path(raw)
    return Path.home() / ".foundry" / "anvil" / "tmp"
...
def sweep_foundry_anvil_tmp(tmp: Path | None = None) -> dict[str, int]:
    """Reap Anvil dump trees under ``~/.foundry/anvil/tmp``.

    ``--state`` (the demo chain) lives in ``ALIEN_UNIVERSE_ANVIL_STATE_DIR``.
    Foundry *also* writes ``anvil-state-*`` dump directories next to its IPC
    cache and never deletes them. On the oracle host five of those trees reached
    **35 GB** and filled the disk — nginx then truncated responses, which is how
    a demo chain took down unrelated public pages.

    Keep the newest dump dir (for a crash you might actually look at) and a
    handful of files inside it. Everything else goes. Never raises: this runs
    on the monitor tick.
    """
    result = {"removed_dirs": 0, "removed_files": 0, "freed_bytes": 0}
    try:
        root = tmp if tmp is not None else foundry_anvil_tmp_dir()
        if not root.is_dir():
            return result
        keep_dirs = max(0, int(os.environ.get("ALIEN_FOUNDRY_ANVIL_TMP_KEEP_DIRS", "1") or 1))
        keep_files = max(0, int(os.environ.get("ALIEN_FOUNDRY_ANVIL_TMP_KEEP_FILES", "3") or 3))
        limit_mb = max(0, int(os.environ.get("ALIEN_FOUNDRY_ANVIL_TMP_MAX_MB", "256") or 256))

        dumps = [p for p in root.iterdir() if p.name.startswith("anvil-state-")]
        dumps.sort(key=_path_mtime, reverse=True)

        for dump in dumps:
            if not dump.is_dir():
                continue
            files = [p for p in dump.iterdir() if p.is_file()]
            files.sort(key=_path_mtime, reverse=True)
            for stale in files[keep_files:]:
                size = _path_bytes(stale)
                _unlink_path(stale)
                result["removed_files"] += 1
                result["freed_bytes"] += size

        for stale in dumps[keep_dirs:]:
            if not stale.exists():
                continue
            size = _path_bytes(stale)
            _unlink_path(stale)
        ...
        remaining = [
            ...
            if p.name.startswith("anvil-state-") and p.exists()
        ]
        remaining.sort(key=_path_mtime)
        while remaining and _path_bytes(root) > limit:
            victim = remaining.pop(0)
            if victim.is_dir():
                files = [p for p in victim.iterdir() if p.is_file()]
                files.sort(key=_path_mtime)
                if files:
                    f = files[0]
                    size = _path_bytes(f)
                    _unlink_path(f)
...
class VirtualUniverse:
    ...
    def _sweep_foundry_anvil_tmp(self) -> None:
        """Drop Foundry dump trees that are not the demo ``--state`` file.

        Anvil writes a full snapshot under ``~/.foundry/anvil/tmp/anvil-state-*``
        and never reaps it. Those trees are independent of
        ``ALIEN_UNIVERSE_ANVIL_STATE_DIR``; the size cap on ``state.json`` never
        sees them. Five of them reached **35 GB** on the oracle host and filled
        the disk. Safe to delete: anvil does not read them back.
        """
        stats = sweep_foundry_anvil_tmp()
        if stats["freed_bytes"] < 8 * 1024 * 1024:
            return
        note = (f"foundry anvil tmp sweep: freed {stats['freed_bytes'] / 1_048_576:.0f}MB "
                f"({stats['removed_dirs']} dump dirs, {stats['removed_files']} files)")
        print(f"[Universe] {note}")
        notes = getattr(self, "_bootstrap_notes", None)
    ...
    def _recycle_anvil_if_oversized(self) -> None:
        ...
        **1.73 GB** on the oracle host: 3.9 GB of RSS on a box with 12 GB, swap exhausted, and then
        ...
        if self.tick < 2 or self.tick % every != 0:
            return
        # Foundry tmp dumps grow even when state.json is under the cap — sweep
        # them on this interval regardless of the --state file size.
        with contextlib.suppress(Exception):
            self._sweep_foundry_anvil_tmp()
        limit_mb = max(1, int(os.environ.get("ALIEN_ANVIL_STATE_MAX_MB", "64") or 64))
        size = self._anvil_state_bytes()
    ...
    def seed_entities(self):
        ...
        self._seed_hestia_entity()
        self._seed_dolos_entity()
    ...
    def _seed_histor_entity(self) -> None:
        """The MCP transparency log; must exist because UNI ignores build_topology()."""
        if "histor" in self.entities:
            return
        from histor_layers import histor_node_spec

        spec = histor_node_spec(mode="universe")
        ent = EcosystemEntity(
            spec["id"],
            spec["label"],
            "security",
        ...
        ent.metrics = dict(spec.get("metrics") or {})
        ent.status = spec.get("status", "offline")
        ent.color = spec.get("color", "#ffd58a")
        self.entities[ent.id] = ent
    ...
    def _seed_hestia_entity(self) -> None:
        """The hearth; must exist because UNI ignores build_topology()."""
        if "hestia" in self.entities:
            return
        from hestia_layers import hestia_node_spec

        spec = hestia_node_spec(mode="universe")
        ent = EcosystemEntity(
            spec["id"],
            spec["label"],
            "infra",
        ...
        ent.metrics = dict(spec.get("metrics") or {})
        ent.status = spec.get("status", "offline")
        ent.color = spec.get("color", "#ff9a3c")
        self.entities[ent.id] = ent

    def _seed_dolos_entity(self) -> None:
        """Dynamic EVM red team; must exist because UNI ignores build_topology()."""
        if "dolos" in self.entities:
            return
        from dolos_layers import dolos_node_spec

        spec = dolos_node_spec(mode="universe")
        ent = EcosystemEntity(
            spec["id"],
            spec["label"],
            "security",
    ...
    def tick_universe(self) -> dict:
        ...
        # Sweep Foundry dumps + recycle oversized --state even if the chain is
        # currently down: the dumps accumulate either way, and a huge state.json
        # is why the chain cannot come back.
        ...
        apply_buyer_health(summary, scenario_output)
    ...
    def get_topology_links(self) -> list[dict]:
        ...
        if "histor" in self.entities:
            from histor_layers import histor_topology_links

            for link in histor_topology_links():
        ...
        if "basanos" in self.entities:
            ...
            from dolos_layers import dolos_topology_links

            links.extend(basanos_topology_links())
            links.extend(dolos_topology_links())
        if "hestia" in self.entities:
            from hestia_layers import hestia_topology_links
