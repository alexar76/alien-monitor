...
_DEFAULT_LAB_PUBLIC = "http://hunt.modelmarket.dev:9083"
_DEFAULT_LAB_API = "http://hunt.modelmarket.dev:9083"
_DEFAULT_HUNT = "https://hunt.modelmarket.dev"
_DEFAULT_USE = "https://use.modelmarket.dev"


def competing_hub_url() -> str:
    """Public operator edge for Competing Lab Hub (the :9083 process, not hunt HTTPS)."""
    return (
        os.environ.get("ALIEN_COMPETING_HUB_PUBLIC_URL", "").strip()
        or os.environ.get("ALIEN_COMPETING_HUB_URL", "").strip()
        or os.environ.get("ALIEN_LAB_HUB_URL", "").strip()
        or _DEFAULT_LAB_PUBLIC
    )


def competing_hub_api_url() -> str:
    """Hub API origin for federation polls (may be loopback or :9083 on the lab host)."""
    return (
        os.environ.get("ALIEN_COMPETING_HUB_API_URL", "").strip()
        or os.environ.get("ALIEN_LAB_HUB_API_URL", "").strip()
        or _DEFAULT_LAB_API
    )
...
async def enrich_competing_lab_nodes(
    ...
    *,
    ...
    lab = competing_hub_api_url().rstrip("/")
