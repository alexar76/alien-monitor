...
def gaia_node_spec(*, mode: str = "real") -> dict[str, Any]:
    ...
    return {
        ...
        "description": (
            ...
            "and live public-API relays (weather, air, tide, river, marine, grid, "
            "quake, energy) sign every reading with a per-device Ed25519 key; a "
            "plausibility verifier gates payment under Pay-on-Verified escrow, so a "
            "lying sensor automatically refunds the buyer. Loosely-coupled peer: the "
            "ecosystem runs without it, and it runs alone."
        ),
