...
def _first_set(*names: str, default: str = "") -> str:
    """First env var that is SET wins — even when it is set to an empty string.

    An `or` chain cannot express "off": ``ALIEN_DIOSCURI_TELEGRAM_URL: ""`` in
    docker-compose.everything.yml was meant to keep the project's own Telegram out
    of a self-hosted monitor (its Discord sibling on the next line does exactly
    that), but an empty string is falsy, so the chain fell straight through to
    ``DEFAULT_TELEGRAM_CHANNEL_URL`` and the link kept showing. Presence, not
    truthiness, is what an operator setting a variable to nothing is expressing.
    """
    for name in names:
...
def dioscuri_community_links() -> dict[str, str]:
    ...
    github = _first_set(
        "ALIEN_DIOSCURI_GITHUB_URL",
        "DIOSCURI_GITHUB_URL",
        default="https://github.com/alexar76/dioscuri",
    ).strip()
    links: dict[str, str] = {}
    if github:
        links["github"] = github.rstrip("/")
    telegram_bot = _first_set(
        "ALIEN_DIOSCURI_TELEGRAM_BOT_URL",
        "DIOSCURI_TELEGRAM_BOT_URL",
        default=DEFAULT_TELEGRAM_BOT_URL,
    ).strip()
    telegram_channel = _first_set(
        "ALIEN_DIOSCURI_TELEGRAM_CHANNEL_URL",
        "DIOSCURI_TELEGRAM_CHANNEL_URL",
        "ALIEN_DIOSCURI_TELEGRAM_URL",
        "DIOSCURI_TELEGRAM_URL",
        default=DEFAULT_TELEGRAM_CHANNEL_URL,
    ).strip()
    discord = _first_set(
        "ALIEN_DIOSCURI_DISCORD_URL",
        "DIOSCURI_DISCORD_URL",
...
@ttl_cached(env_var="ALIEN_DIOSCURI_CACHE_TTL")
