...
NODE_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "histor": (
        "histor", "ἵστωρ", "histor.modelmarket.dev",
        "transparency log", "mcp transparency log", "tool definition log", "merkle log",
        "хистор", "хистора", "хисторе", "хистором",
        "журнал прозрачности", "журнал определений инструментов",
        "registro de transparencia", "journal de transparence",
        "透明日志", "工具定义透明日志",
    ),
    "hestia": (
        "hestia", "ἑστία", "hestia.modelmarket.dev",
        "hearth", "hosted runtime", "hosted provider",
        "гестия", "гестии", "гестию", "гестией",
        "очаг", "очага", "очаге", "очагом",
        "хостед рантайм", "хостинг провайдеров",
        "hogar", "hogar aislado", "runtime alojado",
        "âtre", "âtre isolé", "runtime hébergé",
        "炉灶", "炉灶运行时", "托管运行时",
    ),
}
...
# Nodes whose Latin name is the start of everyday words — "histor" in history / historial /
# historique — match on word boundaries only; a substring match sent "show the price history of
# GAIA" to HISTOR.
_WORD_BOUNDARY_NODES = {"histor"}


def _mentions(node_id: str, form: str, *texts: str) -> bool:
    if node_id in _WORD_BOUNDARY_NODES and form.isascii():
        pattern = re.compile(rf"(?<![a-z0-9]){re.escape(form)}(?![a-z0-9])")
        return any(pattern.search(t) for t in texts)
    return any(form in t for t in texts)
...
def _match_live_node(q: str, state: dict | None) -> tuple[int, str] | None:
    ...
    for n in state.get("nodes") or []:
        ...
        for raw in candidates:
            ...
            if len(form) < 3:
                continue
            if _mentions(nid, form, q, q_latin):
                score = len(form)
            elif nid in _WORD_BOUNDARY_NODES:
                continue  # no folded near-spellings either: "история" is not HISTOR
            else:
