...
#: The lottery's own lifetime counters. The contract comments call them
#: "lifetime economy counters (for the monitor / showcase)" — they were added for this
#: dashboard and the dashboard never read them, showing derived guesses instead.
SEL_PARTICIPANTS_COUNT = "0ef2e857"     # participantsCount(uint256)
SEL_TOTAL_FUNDING = "fe47a8a7"          # totalFunding()
SEL_TOTAL_PRIZES_PAID = "7b61adc3"      # totalPrizesPaid()
SEL_TOTAL_OPEX_ACCRUED = "b266374f"     # totalOpexAccrued()
SEL_TOTAL_TICKET_REVENUE = "973d1d99"   # totalTicketRevenue()
...
async def read_lottery_economy(
    call: Caller, address: str, *, round_id: int, decimals: int = 6
) -> dict:
    """The lottery's lifetime counters, in the settlement token's own unit.

    LIFETIME, not per-window: the contract keeps no 24h aggregate, so these must never be
    published under a 24h name. The monitor used to synthesise `payouts_24h` / `opex_24h`
    / `funding_24h` as the hub's channel volume times a constant (0.03 / 0.02 / 0.20) —
    numbers that contradicted this contract, which on Base has never opened a round.
    """
    scale = 10 ** decimals
    async def whole(selector: str, *args: str) -> float:
        return round(decode_uint(await call(address, call_data(selector, *args))) / scale, 6)

    return {
        "players": decode_uint(
            await call(address, call_data(SEL_PARTICIPANTS_COUNT, uint_arg(round_id)))
        ),
        "funding_total": await whole(SEL_TOTAL_FUNDING),
        "prizes_paid": await whole(SEL_TOTAL_PRIZES_PAID),
        "opex_accrued": await whole(SEL_TOTAL_OPEX_ACCRUED),
        "ticket_revenue": await whole(SEL_TOTAL_TICKET_REVENUE),
    }
...
# topic0 is DERIVED from the signature string, never written down. A hand-copied topic is
# exactly how this file spent its life over-reporting open channels: EV_CHANNEL_SETTLED was
# pinned to keccak("ChannelSettled(bytes32,uint256,uint256,address)"), while
# AIMarketEscrow.sol:82-88 declares FIVE parameters (usedRecipient AND refundRecipient), whose
# topic0 is 0x02b718d7… . The filter therefore never matched a settlement, every settled
# channel kept counting as open, and the tests passed because they use these constants as
# opaque dict keys — a wrong constant and a right one are indistinguishable to them.
#
# Deriving costs one keccak at import and makes the failure impossible: change the event in
# Solidity and the signature string here is what has to change, in the same words.
EVENT_SIGNATURES = {
    "opened": "ChannelOpened(bytes32,address,address,uint256,uint256)",
    "settled": "ChannelSettled(bytes32,uint256,uint256,address,address)",
    "refunded": "ChannelRefunded(bytes32,uint256,string)",
    "expired": "ChannelExpiredAndSettled(bytes32,uint256,uint256)",
}


def event_topic(signature: str) -> str:
    """keccak(signature) as a 0x topic0."""
    from eth_utils import keccak as _keccak

    return "0x" + _keccak(signature.encode()).hex()


EV_CHANNEL_OPENED = event_topic(EVENT_SIGNATURES["opened"])
EV_CHANNEL_SETTLED = event_topic(EVENT_SIGNATURES["settled"])
EV_CHANNEL_REFUNDED = event_topic(EVENT_SIGNATURES["refunded"])
EV_CHANNEL_EXPIRED = event_topic(EVENT_SIGNATURES["expired"])
