...
#: A deployment that runs only ONE of the two realms says so here. Not every ecosystem has
#: a bubble: independentai.network runs the live map alone, and because the default is
#: relative rather than empty, its UI advertised a UNI map at a path where nothing is served
#: — a dead "UNI map →" link and a UNI button that did nothing when clicked (measured
#: 2026-09-08: monitor-uni.independentai.network answered 200 from nginx with no process
#: behind it). An unset variable still means "use the relative default", because that is
#: right for a single-host clone serving both maps behind paths; only this explicit refusal
#: means "there is no other realm here".
OFF_VALUES = frozenset({"off", "none", "disabled", "0", "false", "no"})
...
def _map_url(env_name: str, default: str) -> str:
    """The other realm's map, or "" when the operator declared there is none."""
...
def universe_map_url() -> str:
    return _map_url("ALIEN_UNIVERSE_MAP_URL", DEFAULT_UNIVERSE_MAP)


def live_map_url() -> str:
    return _map_url("ALIEN_LIVE_MAP_URL", DEFAULT_LIVE_MAP)
...
def realm_links(mode: str) -> dict[str, object]:
    ...
    if mode == "universe":
        live = live_map_url()
        payload: dict[str, object] = {"realm": "uni"}
        hub = uni_hub_url()
        if hub:
            payload["hub_url"] = hub
        # No `other` at all when this deployment has no live map. The UI reads the ABSENCE
        # as "do not offer that realm" — offering it with no destination is what produced a
        # button that could only disappoint.
        if live:
            payload["other"] = {"realm": "live", "map_url": live}
        return payload
    if mode == "real":
        uni = universe_map_url()
        payload = {"realm": "live"}
        if uni:
            payload["other"] = {"realm": "uni", "map_url": uni}
        return payload
    return {}
