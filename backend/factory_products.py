...
def published_app_count(nodes: list[dict]) -> int:
    """Storefront products that are apps rather than templates, from the assembled graph.

    The summary's ``apps_online`` was a literal ``0`` in LIVE — a headline cell that could
    never move, beside a factory node already reporting nineteen published products.
    """
    for n in nodes:
        if n.get("group") == "cluster" and n.get("id") == CATALOG_CLUSTER_ID:
            m = n.get("metrics") or {}
            return max(0, int(m.get("count") or 0) - int(m.get("templates") or 0))
    return 0
