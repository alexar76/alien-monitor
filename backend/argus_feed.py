...
def set_argus_run(payload: dict) -> None:
    ...
    cleaned = _clean_run(payload)
    _LIVE["run"] = cleaned
    # Multi-instance roster (Factory Products pattern) — fail-soft.
    try:
        from argus_roster import note_run

        note_run(payload if isinstance(payload, dict) else {}, cleaned)
    except Exception:
        pass
...
def clean_argus_run(payload: dict) -> dict:
    """Public sanitizer for roster / API consumers."""
    return _clean_run(payload)
