...
def hub_events_to_activity(hub_payload: dict[str, Any]) -> tuple[list[dict], dict[str, Any]]:
    ...
    if isinstance(summary, dict):
        hints["invocations_24h"] = int(
            summary.get("invocations_24h")
            or summary.get("total_invocations")
            or 0
        )
        hints["channels_open"] = int(summary.get("open_channels") or summary.get("channels_open") or 0)
        # Hub /stats/live exposes settled/closed channel volume — not bare volume_usd.
        hints["volume_24h"] = float(
            summary.get("volume_usd")
            or summary.get("volume_24h")
            or summary.get("settled_volume_usd")
            or summary.get("closed_volume_usd")
            or 0
        )
    ...
    # When no channel has settled yet, still surface invoke list prices so LIVE
    # is not stuck at $0 while the activity feed is clearly moving.
    if hints["volume_24h"] <= 0 and event_volume > 0:
        hints["volume_24h"] = round(event_volume, 4)
    return events_out, hints
...
def build_real_summary(
    *,
    ...
    onchain_tx = int(
        chain.get("tx_count")
        or (evm.get("tx_count") if isinstance(evm, dict) else 0)
        or hub_hints.get("onchain_tx_count")
        or 0
    )
