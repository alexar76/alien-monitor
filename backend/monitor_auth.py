...
def trusted_proxies() -> frozenset[str]:
    """Addresses whose ``X-Forwarded-For`` this monitor is willing to believe.

    Empty by default: a deployment with no declared proxy trusts NOTHING and keys on the
    raw peer. ``*`` is refused on purpose — blanket-trusting the header lets any caller
    name its own address and hand itself a fresh rate-limit bucket per request, which is
    strictly worse than the one-shared-bucket problem this exists to fix.
    """
    out: set[str] = set()
    for part in (os.environ.get("ALIEN_TRUSTED_PROXIES") or "").split(","):
        entry = part.strip()
        if not entry:
            continue
        if entry == "*":
            logger.error(
                "ALIEN_TRUSTED_PROXIES=* is refused: blanket-trusting X-Forwarded-For lets "
                "every caller forge its own client address. List the proxy address(es) "
                "explicitly (e.g. 127.0.0.1)."
            )
            continue
        out.add(entry)
...
def client_address(request: Request) -> str:
    """The address the public rate limits key on, honouring a declared reverse proxy.

    Behind ``deploy/nginx/monitor.modelmarket.dev.conf`` this process's peer is always the
    proxy (127.0.0.1 — the container runs with ``network_mode: host``), so keying on
    ``request.client.host`` alone put THE WHOLE INTERNET IN ONE BUCKET: 20 AI questions a
    minute and 10 session attempts per five minutes, shared by every visitor. One caller
    then 429s the public AI panel for everyone and locks the operator out of
    ``/api/auth/session``.

    ``X-Forwarded-For`` is consulted ONLY when the immediate peer is a declared proxy, so
    a direct caller cannot forge an address. The chain is walked RIGHT TO LEFT because
    nginx uses ``$proxy_add_x_forwarded_for``, which APPENDS the real peer to whatever the
    client sent: reading left-to-right would take the forged value, while the rightmost
    untrusted hop is the address nginx itself observed. Same shape as the hub's
    ``_client_address`` and the mesh's ``client_ip``.
    """
    peer = ""
    ...
    except Exception:  # noqa: BLE001 — a malformed header must not break the limiter
        raw = ""
    for hop in reversed([h.strip() for h in str(raw).split(",")]):
        if hop and hop not in proxies:
            return hop
    # Every hop is a trusted proxy (or the header is absent): the proxy itself is the only
    # address this request can be attributed to. Not exempt — one shared bucket.
    return peer
