...
class HubSpawner:
    ...
    def tick(self, current_tick: int, vu: VirtualUniverse) -> dict | None:
        ...
        # Was self._spawned_hubs — an attribute that is never created, so every spawn
        # died with AttributeError inside the scenario's except-and-print, and the
        # FEDERATION → MATURITY gate (>= 3 hubs) could never be reached.
        self.spawned_hubs.append({
            "name": name,
            "url": hub_url,
        ...
        self._materialize_entity(name, capabilities, vu)
    ...
    def _materialize_entity(self, name: str, capabilities: list[dict], vu: VirtualUniverse) -> None:
        ...
        # NO address and no measured figures. This hub is a name from HUB_NAMES with a
        # themed capability list — the announce carries a loopback URL to keep the real
        # hub's federation path exercised, but no process ever listens there. The card used
        # to print that URL as this node's "internal address" and to sit `peers: 1` /
        # `invocations_24h: 0` in the metrics grid, so a fixture read exactly like a polled
        # peer that happened to be idle. Both were literals: the peer count could never
        # move and the invocation count could never rise, and `peers` fed nodeActivity() in
        # the reputation graph, letting an invented hub earn trust.
        entity.simulated = True
        entity.simulated_note = "UNI scenario fixture — no service answers for this hub"
        entity.metrics = {"capabilities": len(capabilities)}

        # A random box around the hub could land a hub inside the hub's own sphere
        # (and re-rolled on every respawn). Deterministic ring slot instead, keyed by
        # the hub's own name so the same hub always comes back to the same place.
        from ecosystem_layout import federated_hub_position

        try:
            slot = HUB_NAMES.index(name)
        except ValueError:
            slot = sum(ord(ch) for ch in name)
        # Anchored on the federation node, never on the hub: a ring hung off the hub
        # itself runs straight through the factory/contract shelf.
        anchor = vu.entities.get("federation")
        entity.position = federated_hub_position(
            slot, federation_pos=dict(anchor.position) if anchor else None
        )
