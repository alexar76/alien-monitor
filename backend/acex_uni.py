...
def _has_code(w3: Any, address: str | None) -> bool:
    """Is there still a contract at this address on the chain we are talking to now?"""
    if not w3 or not address:
        return False
    try:
        return bool(w3.eth.get_code(w3.to_checksum_address(address)))
    except Exception:
        return False
...
def seed_acex_markets(u: VirtualUniverse) -> bool:
    ...
    # Idempotent against the CHAIN, not against this file. Trusting the file alone is how
    # ACEX died quietly: the bubble Anvil lost the block range where SNTL / ATLS / GAIA
    # were deployed, `acex_uni_state.json` still pinned their addresses, seeding therefore
    # decided it had nothing to do, and every trade tick swapped against three addresses
    # with no code — "Could not transact with/call contract function" every 30 ticks,
    # while the map went on showing 1136 trades and 281 200 of volume from the file.
    live_pools = [p for p in (state.get("pools") or []) if _has_code(u._w3, p.get("share"))]
    lost = len(state.get("pools") or []) - len(live_pools)
    if lost:
        print(
            f"[ACEX-UNI] {lost} CapShare token(s) are gone from this chain — re-seeding "
            f"(cumulative volume/trades are kept; they happened)"
        )
        state["pools"] = live_pools
        save_acex_state(u, state)
    if live_pools and len(live_pools) >= len(DEMO_LISTINGS):
        return True
...
def tick_acex_trades(u: VirtualUniverse) -> None:
    ...
    state = load_acex_state(u)
    pools = [p for p in (state.get("pools") or []) if _has_code(u._w3, p.get("share"))]
    if not pools:
        # Nothing to trade against. Say so once per interval instead of reverting into the
        # log, and let the next seed pass rebuild the pools.
        if state.get("pools"):
            print("[ACEX-UNI] no live CapShare pools on this chain — waiting for a re-seed")
        return
...
def acex_metrics_for_monitor(u: VirtualUniverse) -> dict[str, Any]:
    ...
    live = [p for p in pools if _has_code(u._w3, p.get("share"))]
    return {
        # `volume_usdc` and `trades` are CUMULATIVE counters in acex_uni_state.json, with
        # no time window anywhere near them. Published as `volume_24h` they put 281 200 of
        # lifetime turnover on a card labelled "24h" — the same defect the header carried
        # until 2026-09-07, in a different corner.
        "volume_total_usd": round(float(state.get("volume_usdc") or 0), 2),
        "trades_total": int(state.get("trades") or 0),
        "listings": len(live),
        # What can actually be traded right now, which is what "active" has to mean.
        "pools_active": len(live),
        "pools_recorded": len(pools),
    }
