...
class EcosystemSimulator:
    ...
    def step(self) -> dict:
        ...
        apply_hub_affiliation(nodes, primary_hub_url=HUB_URL)
...
**Platon Shadow Oracle** (external, https://oracles.modelmarket.dev) — a 32D dynamical
...
def _universe_fast_snapshot() -> dict:
    ...
    apply_atlas_graph(graph_nodes, mode="universe")
    from factory_agents import apply_factory_agents_graph

    apply_factory_agents_graph(graph_nodes, mode="universe")
    from momus_status import apply_momus_graph
    from treasury_status import apply_treasury_graph

    apply_momus_graph(graph_nodes, mode="universe")
    apply_logos_to_nodes(graph_nodes, fetch_logos_status_sync())
    apply_treasury_graph(graph_nodes, mode="universe")
    from bridges_status import apply_bridges_graph

    apply_bridges_graph(graph_nodes, mode="universe")
    from themis_layers import apply_themis_graph

    apply_themis_graph(graph_nodes, mode="universe", tick=u.tick, hub_payload=None)
    ensure_factory_clusters(graph_nodes, graph_links, APP_URL, catalog_timeout=5.0)
    apply_hub_affiliation(graph_nodes, primary_hub_url=HUB_URL)
    return {
        "tick": u.tick,
        "ts": datetime.now(timezone.utc).isoformat(),
        "nodes": graph_nodes,
    ...
    async with _state_fetch_lock:
        if mode == "universe":
            ...
            try:
                ...
                pass
            apply_hub_affiliation(state.get("nodes") or [], primary_hub_url=HUB_URL)
        elif mode == "real":
...
async def get_topology():
    ...
    if sm == "universe":
        ...
        apply_themis_graph(nodes, mode="universe", tick=u.tick, hub_payload=None)
...
def _fallback_answer(
    ...
    if locale == "ru":
        ...
        return "Спросите о хабе, SKOPOS, Metis, THEMIS, DIOSCURI, MOMUS, контрактах, плагинах, mesh или ACEX." + live_hint
    if locale == "es":
        ...
        return "Pregunta sobre hub, SKOPOS, Metis, THEMIS, DIOSCURI, MOMUS, contratos, plugins o mesh." + live_hint
    # English
    ...
    return "Ask about the hub, SKOPOS, Metis, THEMIS, DIOSCURI, MOMUS, contracts, plugins, desktop apps, service mesh, ACEX, SDKs, or current ecosystem state." + live_hint
