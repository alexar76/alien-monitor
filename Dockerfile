...
# The satellite catalog the AI assistant's prompt context is built from. Baked in as a FLOOR that
# always matches this image: the deployed container had NO copy of it at all, so
# build_ecosystem_registry_context() returned an empty string and the bot answered that MOMUS — and in
# fact every satellite in the map — did not exist. docker-compose.prod.yml also MOUNTS the host's copy
# at /app/scripts/satellite-map.yaml, which ecosystem_registry.py prefers, so an operator who adds a
# satellite does not need an image rebuild (a restart is enough — the loader lru_caches it).
COPY scripts/satellite-map.yaml ./backend/data/satellite-map.yaml
...
COPY acex/contracts/evm/ ./acex/contracts/evm/
...
RUN cd /app/acex/contracts/evm && \
    (test -d lib/forge-std || forge install foundry-rs/forge-std@v1.9.4 OpenZeppelin/openzeppelin-contracts@v5.0.2 --no-git) && \
    forge build src/

WORKDIR /app/backend
EXPOSE 9100
# Liveness, not feature-completeness. The old form also required `blockchain_ready` in
# universe mode, i.e. the EMBEDDED anvil that this container starts for the simulated
# on-chain layer. When anvil is slow or absent the monitor still serves topology, summary and
# the live websocket perfectly — but the container reported unhealthy forever (600 consecutive
# failures on 2026-08-24), which trains everyone to ignore the one signal that should mean
# "this is down". The simulated chain's state is reported in the payload and belongs on a
# dashboard, not in a binary liveness gate: a degraded feature is not a dead process.
HEALTHCHECK --interval=30s --timeout=10s --start-period=90s --retries=5 \
  CMD python -c "import json,urllib.request; d=json.loads(urllib.request.urlopen('http://127.0.0.1:9100/api/health',timeout=5).read()); exit(0 if d.get('status')=='ok' else 1)"
