...
### 🔎 Escáner de cadena (SCAN)

Un pequeño explorador de bloques para **la cadena propia de este reino**. La burbuja UNI corre su
propio Anvil y, hasta que esto existió, el monitor jamás había leído un bloque. Se abre con el
botón **SCAN** de la barra de control.

| Endpoint | Qué responde |
|----------|--------------|
| `GET /api/chain/scan?limit=40` | transacciones recientes, las más nuevas primero |
| `GET /api/chain/tx/{hash}` | una transacción con su recibo (estado / gas / logs) |
| `GET /api/chain/address/{addr}` | saldo, nonce, tamaño del código, transacciones recientes |

- **Ligado al reino** — lee el `ALIEN_EVM_RPC` de su propio proceso: el monitor UNI escanea la
  burbuja y el monitor LIVE su propia cadena.
- **Autenticación de lectura**, como el resto de rutas de lectura (abierto con `ALIEN_PUBLIC_READ=1`).
- **Nombres, nunca conjeturas** — las direcciones salen de `config/deployments/*.json` de este reino
  y del `universe_config.json` de la burbuja; los métodos, de una tabla de selectores de 4 bytes
  construida sobre los ABI reales. Un selector desconocido se queda en hex.
- **Aviso de identidad de cadena** — si el RPC responde con un chainId distinto al de la red
  configurada, el escaneo y `/api/chain/status` devuelven `chain_mismatch` y el panel muestra un aviso.
- **Nunca da 500** — cualquier fallo es un escaneo vacío con `note`; un escáner no puede tumbar el
  panel.

Para qué sirve: una dirección con `code_size: 0` que recibe un flujo constante de llamadas *exitosas*
es un componente operando un contrato que ya no existe — una llamada a una dirección sin código no
revierte. Así el relayer de la lotería UNI pasó nueve días llamando a `openRound()` al vacío.
