...
class TestSharedPeersAreEdgesNotCopies:
    ...
    def test_our_own_hub_never_becomes_somebody_elses_moon(self, monkeypatch):
        ...
        # `new.example` declares a capability because this test is about DEDUP, not about
        # participation: a second-hop node with an empty catalogue is correctly not drawn
        # (see test_sphere_participation), and leaving it at zero here would make this test
        # pass or fail for a reason that has nothing to do with what it checks.
        theirs = {
    ...
    def test_our_own_hub_becomes_an_edge_even_with_an_empty_catalogue(self, monkeypatch):
        """The sphere rule must never be asked about a node the map already draws.

        `ours.example` is US. It has no `capabilities_count` in the stranger's peer list, so
        a participation test applied before the dedup step silenced it — and with it the
        edge that says "they know us", which is also what keeps our own hub from being drawn
        a second time as somebody else's moon. A relationship visible from both ends is a
        fact about our federation, not a claim needing evidence.
        """
        import asyncio

        theirs = {
            "name": "Their Hub", "hub_version": "3.2.1",
            "peers": [{"url": "https://ours.example", "name": "Ours"}],  # no caps at all
        }
        ...
        assert not any("ours.example" in n["id"] for n in res["nodes"])
...
def test_a_pending_hub_seen_again_at_hop_2_becomes_an_edge(monkeypatch):
    """A hub knocking for admission is drawn later in the SAME pass.

    Measured on `monitor.attestedmemory.net`: `modelmarket.dev` was a pending hub there and
    also a peer of Signal Hunt. Its second-hop copy carried no capability count, so the
    participation rule dropped it — and with it the edge showing that those two hubs know
    each other, about a node the map was drawing anyway two rings out.
    """
    import asyncio

    # A neutral address on purpose: `modelmarket.dev` is the apex in the monitor's own
    # identity tables, so a stranger URL is what exercises the pending path rather than the
    # apex fold.
    hunt = {
        "name": "Signal Hunt Hub", "hub_version": "3.2.1",
        "peers": [{"url": "https://knocker.example", "name": "Knocker Hub"}],
    }

    async def fake_get_json(client, url, **kwargs):
        if url.endswith("/federation/peers"):
            return {
                "peers": [{"url": "https://hunt.example"}],
                "pending": [{"url": "https://knocker.example", "name": "Knocker Hub"}],
            }
        if "hunt.example" in url and "well-known" in url:
            return hunt
        return None
    ...
    ids = [n["id"] for n in res["nodes"]]
    pending_id = next((i for i in ids if "knocker.example" in i), "")
    assert pending_id, "the pending hub must still be drawn"
    # …exactly once, and the second-hop sighting is an edge to it.
    assert sum(1 for i in ids if "knocker.example" in i) == 1
    shared = [l for l in res["links"] if l.get("kind") == "shared"]
    assert pending_id in [l["target"] for l in shared], (
        "the second-hop copy of a knocking hub must fold into it, not vanish"
    )


def test_a_folded_copy_hands_over_its_on_chain_declaration(monkeypatch):
    """The fold must move the evidence, not discard it.

    A hub re-exports what its peers declared, so a second-hop sighting is often the only
    route by which an address reaches this map at all. `modelmarket.dev` reached
    `monitor.attestedmemory.net` through Signal Hunt with three addresses verified against
    Base, folded into its own pending sphere — and the sphere showed nothing, because the
    copy that carried the declaration was thrown away after the edge was drawn.
    """
    import asyncio

    declaration = {
        "chain_id": 8453, "verified": True,
        "entries": [{"role": "escrow", "address": "0x" + "a" * 40, "verified": True}],
    }
    hunt = {
        "name": "Signal Hunt Hub", "hub_version": "3.2.1",
        "peers": [{"url": "https://knocker.example", "name": "Knocker Hub",
    ...
    monkeypatch.setattr(hub_discovery, "_get_json", fake_get_json)
    monkeypatch.setattr(hub_discovery, "url_is_safe", lambda *a, **k: True)
    # Verified, so the declaration that reaches the node is one we checked.
    monkeypatch.setattr(
        hub_discovery.foreign_contracts, "verify",
        lambda client, decls: _verified_all(decls),
    )
    res = asyncio.run(hub_discovery.discover_async("https://ours.example"))

    knocker = next(n for n in res["nodes"] if "knocker.example" in n["id"])
    assert knocker.get("contracts"), "the folded copy's declaration must reach the sphere"
    assert knocker["contracts"]["entries"][0]["address"] == "0x" + "a" * 40


async def _verified_all(declarations):
    """Every declared address, marked verified — stands in for the RPC."""
    out = {}
    for declaration in declarations.values():
        for entry in declaration.get("entries") or []:
            out[entry["address"].lower()] = {
                "verified": True, "reason": "contract deployed", "contract": True,
            }
    return out


def test_a_nodes_own_declaration_outranks_a_strangers_re_export():
    """Never overwrite: a node's own well-known beats somebody's copy of it."""
    mine = {"entries": [{"role": "escrow", "address": "0x" + "1" * 40}]}
    theirs = {"entries": [{"role": "escrow", "address": "0x" + "2" * 40}]}
    nodes = [{"id": "n1", "contracts": mine}]
    assert hub_discovery._attach_declaration("n1", theirs, nodes) is False
    assert nodes[0]["contracts"] is mine

    bare = [{"id": "n2"}]
    assert hub_discovery._attach_declaration("n2", theirs, bare) is True
    assert bare[0]["contracts"] is theirs
    # Nothing usable, nothing attached.
    assert hub_discovery._attach_declaration("n2", {"entries": []}, bare) is False
    assert hub_discovery._attach_declaration("nope", theirs, bare) is False


def test_a_hubs_provider_and_its_peers_share_one_slot_allocation():
    """A hub's PROVIDER and its PEERS must not be handed the same orbit slot.

    Measured live on monitor.independentai.network (2026-09-08): Signal Hunt Hub had one
    provider ("Signal Hunt", placed by discovery, which counts every sibling) and two peers
    (placed by the merge pass, which used to collect `peer_hub_node` alone and so counted
    only itself). Both took slot 0 of the same shell and were drawn 0.971 apart while the
    other pairs sat 3.975 and 4.479 out — one bubble inside another, which is what "THEMIS
    is glued to Signal Hunt" looks like on screen.

    The invariant is not a distance constant: it is that the hub's children occupy DISTINCT
    slots of the same ring, so the assertion is against the ring's own geometry for that
    count. A future ring can be any shape and this still holds.
    """
    import sys
    from pathlib import Path
    ...
    hub_pos = {"x": 2.0, "y": -18.0, "z": 8.0}
    nodes = [
        {"id": "hub", "label": "Hub", "group": "core", "position": {"x": 0, "y": 0, "z": 0}},
        {
            "id": "peer-hub", "label": "Peer Hub", "group": "peer_hub", "role": "hub",
            "status": "active", "position": dict(hub_pos),
        },
        # Already on the graph, placed by discovery counting all four siblings.
        {
            "id": "provider:peer-hub:operator", "label": "Their App",
            "group": "peer_hub_provider", "parent_id": "peer-hub",
            "position": peer_hub_child_position(hub_pos, 0, 4),
        },
    ]
    disc = {
        "nodes": [
            {
                "id": f"fedchild:https://p{i}.example", "label": f"Peer {i}",
                "group": "peer_hub_node", "parent_id": "peer-hub",
                "url": f"https://p{i}.example", "status": "idle",
                "position": dict(hub_pos),
            }
            for i in range(3)
        ],
        "links": [],
    ...
    kids = [n for n in nodes if str(n.get("parent_id") or "") == "peer-hub"]
    assert len(kids) == 4, [n["id"] for n in kids]
    slots = {
        (p["x"], p["y"], p["z"])
        for p in (peer_hub_child_position(hub_pos, i, len(kids)) for i in range(len(kids)))
    }
    used = [(n["position"]["x"], n["position"]["y"], n["position"]["z"]) for n in kids]
    assert len(set(used)) == len(kids), f"two children share a slot: {used}"
    assert set(used) == slots, "children are off the ring their own hub allocates"
    # And the gap that was 0.971 is now whatever the ring gives — never smaller.
    ring = sorted(slots)
    floor = min(
        _dist({"x": a[0], "y": a[1], "z": a[2]}, {"x": b[0], "y": b[1], "z": b[2]})
        for i, a in enumerate(ring) for b in ring[i + 1:]
    )
    worst = min(
        _dist({"x": a[0], "y": a[1], "z": a[2]}, {"x": b[0], "y": b[1], "z": b[2]})
        for i, a in enumerate(used) for b in used[i + 1:]
    )
    assert worst >= floor - 1e-6, f"closest pair {worst} is tighter than the ring's own {floor}"
