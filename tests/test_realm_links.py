...
class TestADeploymentWithOnlyOneRealm:
    """independentai.network runs the live map alone — and said otherwise.

    Because the default was relative rather than empty, the live map advertised a UNI map at
    `/monitor/`, the operator env pointed it at a subdomain nginx answers 200 for with no
    process behind it, and the UI drew a "UNI map →" link plus a UNI button into a realm this
    deployment does not have. An operator has to be able to say "there is no other realm".
    """

    def test_off_means_there_is_no_other_realm(self, monkeypatch):
        for value in ("off", "none", "OFF", "disabled", "0", "false", "no"):
            monkeypatch.setenv("ALIEN_UNIVERSE_MAP_URL", value)
            links = realm_links("real")
            assert links["realm"] == "live"
            assert "other" not in links, value

    def test_it_works_the_other_way_round_too(self, monkeypatch):
        """A bubble-only deployment must be able to hide the live map the same way."""
        monkeypatch.setenv("ALIEN_LIVE_MAP_URL", "off")
        monkeypatch.delenv("ALIEN_UNI_HUB_URL", raising=False)
        links = realm_links("universe")
        assert links == {"realm": "uni"}

    def test_the_bubble_hub_link_survives_the_refusal(self, monkeypatch):
        monkeypatch.setenv("ALIEN_LIVE_MAP_URL", "off")
        monkeypatch.setenv("ALIEN_UNI_HUB_URL", "https://uni.example.dev/")
        links = realm_links("universe")
        assert links["hub_url"] == "https://uni.example.dev"
        assert "other" not in links

    def test_unset_still_means_the_relative_default(self, monkeypatch):
        """A single-host clone serving both maps behind paths must not lose its link."""
        monkeypatch.delenv("ALIEN_UNIVERSE_MAP_URL", raising=False)
        assert realm_links("real")["other"] == {"realm": "uni", "map_url": "/monitor/"}

    def test_a_real_url_is_still_a_real_url(self, monkeypatch):
        monkeypatch.setenv("ALIEN_UNIVERSE_MAP_URL", "https://monitor-uni.example.dev/")
        assert realm_links("real")["other"] == {
            "realm": "uni", "map_url": "https://monitor-uni.example.dev/",
        }
