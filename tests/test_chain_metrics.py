...
def test_hub_events_to_activity():
    ...
    assert hints["invocations_24h"] == 7
    assert hints["channels_open"] == 5
    assert hints["capabilities"] == 85
    assert hints["peers"] == 9
    assert hints["volume_24h"] == 1.5
    assert len(events) == 1
    assert events[0]["agent"] == "peer-a"


def test_quiet_day_never_borrows_the_lifetime_counters():
    """A 24h cell must not fall back to an all-time total when the window is empty.

    Measured on the UNI map 2026-09-07: the hub reported ``invocations_24h: 0`` and
    ``settled_volume_usd: 0.06`` (a lifetime channel figure), and the panel printed
    215 invocations / $0.06 "24h" — every event ten days old.
    """
    payload = {
        "summary": {
            "total_invocations": 215,
            "invocations_24h": 0,
            "volume_24h_usd": 0.0,
            "settled_volume_usd": 0.06,
            "open_channels": 0,
        },
        "events": [{"id": "e1", "price_usd": 0.004, "capability_id": "x", "timestamp": _iso(-864000)}],
    }
    _events, hints = hub_events_to_activity(payload)
    assert hints["invocations_24h"] == 0
    assert hints["volume_24h"] == 0.0
    # Not discarded — just no longer wearing a 24h label.
    assert hints["volume_lifetime_usd"] == 0.06


def test_event_fallback_only_counts_the_window():
    """A hub too old to report windowed figures: derive them, but window the events."""
    payload = {
        "summary": {"total_invocations": 10, "open_channels": 1, "settled_volume_usd": 9.99},
        "events": [
            {"id": "fresh", "price_usd": 0.25, "capability_id": "x", "timestamp": _iso(-3600)},
            {"id": "stale", "price_usd": 5.00, "capability_id": "x", "timestamp": _iso(-200000)},
        ],
    }
    _events, hints = hub_events_to_activity(payload)
    assert hints["volume_24h"] == 0.25
    assert hints["invocations_24h"] == 1
    assert hints["volume_lifetime_usd"] == 9.99


def test_summary_carries_the_window_and_the_lifetime_total():
    summary = build_real_summary(
        tick=1,
        hub_hints={"invocations_24h": 4, "volume_24h": 0.0, "volume_lifetime_usd": 3.32},
        mesh_stats={"agents_total": 3},
        chain={"evm": {"connected": True, "block": 5, "gas_gwei": 0.006}},
    )
    assert summary["total_volume_usd"] == 0.0
    assert summary["volume_window"] == "24h"
    assert summary["volume_lifetime_usd"] == 3.32
    assert summary["agents_online"] == 3


def test_summary_reports_escrow_tvl_read_on_chain():
    summary = build_real_summary(
        tick=1,
        hub_hints={},
        mesh_stats=None,
        chain={
            "evm": {"connected": True, "escrow_tvl_usd": 12.5, "escrow_tvl_token": "USDC"},
            "escrow": {"tvl_usd": 12.5, "token": "USDC"},
        },
    )
    assert summary["tvl_usd"] == 12.5
    assert summary["tvl_token"] == "USDC"


def test_unknown_onchain_tx_count_is_not_reported_as_zero():
    """LIVE has no transaction counter; "--" is honest, 0 is a claim."""
    summary = build_real_summary(
        tick=1, hub_hints={}, mesh_stats=None,
        chain={"evm": {"connected": True, "block": 5}},
    )
    assert summary["onchain_tx_count"] is None

    counted = build_real_summary(
        tick=1, hub_hints={}, mesh_stats=None,
        chain={"tx_count": 0, "evm": {"connected": True}},
    )
    assert counted["onchain_tx_count"] == 0


def test_apps_online_comes_from_the_caller_not_a_literal_zero():
    summary = build_real_summary(
        tick=1, hub_hints={}, mesh_stats=None, chain={}, apps_online=17,
    )
    assert summary["apps_online"] == 17
