...
class TestEveryPollerIsCovered:
    ...
    POLLERS = {
        ...
        "basanos_status": ["fetch_basanos_status_sync"],
        "hestia_status": ["fetch_hestia_status_sync"],
        "histor_status": ["fetch_histor_status_sync"],
        ...
        "settlement_status": ["fetch_settlement_status_sync"],
        "skopos_status": ["fetch_skopos_status_sync"],
        "treasury_status": ["fetch_treasury_status_sync"],
        # WARDEN has no host of its own: this poller reads the feed side (MOMUS).
        "warden_status": ["fetch_warden_status_sync"],
    }

    # Status modules that touch NO network, with the reason. The TTL exists to stop the 1.5 s
    # tick from hammering somebody's HTTP endpoint; a module that only opens a local file has
    # nothing to hammer, and wrapping it would delay the very artifact it exists to surface.
    # Listed rather than silently skipped — "why isn't this one cached?" must have an answer
    # in the file, or the next reader adds a poller here and calls it precedent.
    LOCAL_READERS = {
        "dolos_status": "reads DOLOS's last_scan.json artifact — a CLI harness with no service "
                        "to poll, so there is no endpoint to protect and no rate limit to hit",
    }
    ...
    def test_no_status_module_was_missed(self):
        ...
        allowed = {"argus_status", "metis_status", "treasury_status"} | set(self.LOCAL_READERS)
        surprises = {m: n for m, n in unexpected.items() if m not in allowed}
        assert not surprises, f"undecided sync pollers: {surprises}"

    @pytest.mark.parametrize("module,reason", sorted(LOCAL_READERS.items()))
    def test_an_uncached_poller_really_touches_no_network(self, module, reason):
        """The exemption is only valid while it stays true — a later edit that adds an HTTP
        call to one of these modules must fail here rather than quietly start hammering."""
        assert len(reason) > 40, f"{module} is exempt without a real reason"
        text = (MONITOR / "backend" / f"{module}.py").read_text(encoding="utf-8")
        for dialer in ("httpx", "requests", "urlopen", "aiohttp", "socket."):
            assert dialer not in text, (
                f"{module} now dials the network ({dialer}) — it needs ttl_cached and a line "
                "in POLLERS, not this exemption"
            )
