...
class TestTopology:
    ...
    def test_total_node_count(self):
        ...
        # A floor plus a required set, not an exact count. The exact count broke every time a
        # satellite was added — it asserted that the ecosystem had stopped growing. What matters is
        # that nothing is MISSING; tests/test_all_components_present.py checks the full roster across
        # all three modes.
        assert len(nodes) >= 25
        ids = {n["id"] for n in nodes}
        for required in ("hub", "factory", "mesh", "momus", "treasury"):
            assert required in ids, f"{required} missing from the topology"
...
class TestAPI:
    ...
    def test_topology_endpoint(self):
        ...
        assert "atlas" in {n["id"] for n in data["nodes"]}
        # A floor plus a required set, not an exact count. The exact count broke every time a
        # satellite was added — it asserted that the ecosystem had stopped growing. What matters is
        # that nothing is MISSING; tests/test_all_components_present.py checks the full roster across
        # all three modes.
        ...
        for required in ("hub", "factory", "mesh", "momus", "treasury"):
            assert required in ids, f"{required} missing from /api/topology"

    def test_topology_and_summary_accept_a_mode_query_param(self):
        """The client sends ?mode= to both; the routes used to declare it on neither.

        /api/topology + /api/summary are the HTTP fallback every anonymous viewer takes on
        a deploy where /api/state is authed. With the parameter ignored, clicking TEST
        rendered a simulated graph under a header of LIVE settlement numbers.
        """
        schema = client.get("/openapi.json").json()
        for path in ("/api/topology", "/api/summary"):
            params = schema["paths"][path]["get"].get("parameters") or []
            assert "mode" in {p["name"] for p in params}, path

    def test_summary_passes_the_requested_mode_through(self, monkeypatch):
        import main

        seen = {}

        async def fake_snapshot(mode=None):
            seen["mode"] = mode
            return {"summary": {"mode": "test", "total_invocations_24h": 0}}

        monkeypatch.setattr(main, "_monitor_state_snapshot", fake_snapshot)
        resp = client.get("/api/summary?mode=test")
        assert resp.status_code == 200
        assert seen["mode"] == "test"
    ...
    def test_ai_ask_public_when_alien_public_read(self, monkeypatch):
        monkeypatch.setenv("AIFACTORY_PROD", "1")
        monkeypatch.setenv("ALIEN_PUBLIC_READ", "1")
    ...
    def test_ai_ask_requires_auth_without_public_read(self, monkeypatch):
        monkeypatch.setenv("AIFACTORY_PROD", "1")
        monkeypatch.delenv("ALIEN_PUBLIC_READ", raising=False)
        resp = client.post("/api/ai/ask", json={"question": "atlas"})
        assert resp.status_code == 401
        resp2 = client.post("/api/ai/ask", json={"question": ""}, headers=_AUTH)
        assert resp2.status_code == 200
...
class TestAIFallback:
    ...
    def test_live_provider_is_known_without_an_llm_key(self):
        state = {
            "tick": 7082,
            "nodes": [
                {"id": "hub", "label": "Independent AI Hub"},
                {
                    "id": "provider:hub:kova-gateway",
                    "label": "KOVA",
                    "group": "peer_hub_provider",
                    "parent_id": "hub",
                    "status": "active",
                    "url": "https://independentai.network/kova/",
                },
            ],
        }
        ans = _fallback_answer("где кова?", locale="ru", state=state, mode="live")
        assert "KOVA" in ans
        assert "Independent AI Hub" in ans           # whose system it lives in
        assert "без статического списка" in ans      # …and that it says where it looked
        assert "tick 7082" in ans

    def test_unknown_question_names_what_is_actually_on_this_map(self):
        """The give-up line used to recite one deployment's product list on every map.

        `monitor.attestedmemory.net` answered «где аттестед мемори?» with "ask about the
        hub, SKOPOS, Metis, THEMIS, DIOSCURI, MOMUS…" — none of which are on that map,
        while its own hub and Memory Market were three nodes away.
        """
        state = {
            "tick": 11,
            "nodes": [
                {"id": "hub", "label": "Attested Memory Hub", "group": "core"},
                {
                    "id": "provider:hub:memory-market", "label": "Memory Market",
                    "group": "peer_hub_provider", "parent_id": "hub",
                },
            ],
        }
        ans = _fallback_answer("qwerty asdf zxcv", locale="ru", state=state, mode="live")
        assert "Attested Memory Hub" in ans
        assert "Memory Market" in ans
        assert "SKOPOS" not in ans
...
class TestThePublicReadTierIsActuallyRedacted:
    """`/api/state` is documented as "always token-gated in production (even public demo)".
    `/api/pulse/state` returned the IDENTICAL object under the *read* guard, which
    `ALIEN_PUBLIC_READ=1` (set in docker-compose.prod.yml) turns into a no-op — so the dump
    the state guard exists to protect was served anonymously one URL over.

    And the WebSocket tier decision could never redact: `_state_for_ws` redacted only when
    `not ws_authed and not monitor_public_read_allowed()`, which is exactly the case in which
    the handler has already closed the socket with 1008. Dead branch, unredacted stream.
    """

    def test_the_public_pulse_dump_carries_no_internal_diagnostics(self, monkeypatch):
        monkeypatch.setenv("AIFACTORY_PROD", "1")
        monkeypatch.setenv("ALIEN_PUBLIC_READ", "1")
        import main

        # Inject a snapshot that HAS the sensitive fields. Without this the test env has no
        # upstreams configured, so `components` is absent anyway and the assertion below
        # would pass without the projection existing at all.
        async def _snapshot(_mode=None):
            return {
                "tick": 7, "ts": "now", "mode": "real",
                "summary": {"contracts": {"escrow_evm": "0x1"}, "bootstrap": {"x": 1},
                            "payment_recipient": "0x2", "ticks": 3},
                "nodes": [{"id": "hub", "hub_env_snippet": "SECRET=1"}],
                "links": [], "events": [],
                "components": {"hub": {"raw_upstream": True}},
                "errors": ["internal detail"],
            }

        monkeypatch.setattr(main, "_monitor_state_snapshot", _snapshot)
        data = client.get("/api/pulse/state").json()
        assert "contracts" not in (data.get("summary") or {})
        assert "payment_recipient" not in (data.get("summary") or {})
        assert all("hub_env_snippet" not in n for n in data.get("nodes") or [])
        assert "components" not in data, (
            "raw upstream JSON from the internal hub/mesh/factory/Prometheus served "
            "anonymously — this is what require_monitor_state_auth exists to withhold"
        )
        assert "errors" not in data, "internal diagnostics served anonymously"
        # It must still be usable as the Pulse read path.
        assert "nodes" in data or "summary" in data

    def test_an_authenticated_caller_still_gets_the_full_dump(self, monkeypatch):
        monkeypatch.setenv("AIFACTORY_PROD", "1")
        monkeypatch.delenv("ALIEN_PUBLIC_READ", raising=False)
        data = client.get("/api/pulse/state", headers=_AUTH).json()
        assert "nodes" in data or "summary" in data

    def test_the_websocket_redaction_branch_is_reachable(self, monkeypatch):
        """The tier must follow authentication, not a condition that closes the socket."""
        monkeypatch.setenv("AIFACTORY_PROD", "1")
        monkeypatch.setenv("ALIEN_PUBLIC_READ", "1")
        import main
        ...
        sample = {
            ...
            "nodes": [{"id": "hub", "hub_env_snippet": "SECRET=1"}],
            "errors": ["internal detail"],
            "components": {"hub": {"raw": True}},
            "tick": 1,
        }
        out = main._state_for_ws(sample, ws_authed=False)
        assert "errors" not in out, "anonymous WS viewers still receive internal diagnostics"
        assert "contracts" not in (out.get("summary") or {})
        assert "bootstrap" not in (out.get("summary") or {})
        assert "payment_recipient" not in (out.get("summary") or {})
        assert all("hub_env_snippet" not in n for n in out.get("nodes") or [])
        # An authenticated client is unaffected.
        authed = main._state_for_ws(sample, ws_authed=True)
        assert "contracts" in (authed.get("summary") or {})

    @pytest.mark.parametrize("path", ["/api/chain/status", "/api/ai/providers"])
    def test_every_read_route_is_gated_in_a_locked_down_deployment(self, monkeypatch, path):
        """`/api/chain/status` leaked live contract addresses and the RPC endpoint, and drove
        a burst of un-rate-limited JSON-RPC calls, with no `dependencies=` at all — while
        every sibling read route carried the guard."""
        monkeypatch.setenv("AIFACTORY_PROD", "1")
        monkeypatch.delenv("ALIEN_PUBLIC_READ", raising=False)
        assert client.get(path).status_code == 401, f"{path} answers anonymously"
        assert client.get(path, headers=_AUTH).status_code == 200, f"{path} broke for the operator"
