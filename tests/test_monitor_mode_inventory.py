...
def test_resolve_factory_catalog_uses_monitor_catalog(monkeypatch):
    ...
    assert auth is True
    # The storefront grid drops the shipped apps that miss its quality gates, but their
    # detail pages are live and they are not landing-spam — so they are pinned in front
    # (factory_products.enrich_catalog_with_featured). What the monitor catalog DID carry
    # must survive that, unchanged and in order.
    assert catalog is not None
    assert [p["id"] for p in catalog[:len(fp.FEATURED_FACTORY_PRODUCTS)]] == [
        pid for pid, _ in fp.FEATURED_FACTORY_PRODUCTS
    ]
    assert {"id": "prod-a", "name": "Alpha", "category": "saas"} in catalog
    assert catalog[len(fp.FEATURED_FACTORY_PRODUCTS):] == [
...
def test_resolve_factory_catalog_keeps_last_good_cache(monkeypatch):
    ...
    assert first and "prod-keep" in [p["id"] for p in first], (
        "the fetched row must be in the cached catalogue, featured pins or not"
    )
