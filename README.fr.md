...
**Cartes :** **UNI** [https://monitor-uni.modelmarket.dev/](https://monitor-uni.modelmarket.dev/) (`ALIEN_MODE=universe`) · **LIVE** [https://monitor.modelmarket.dev/](https://monitor.modelmarket.dev/) (`ALIEN_MODE=real`). Découpage : [uni-and-live.fr.md](https://github.com/alexar76/aicom/blob/main/docs/uni-and-live.fr.md).
...
<p align="center">
  ...
  <br>
  <sub>Graphe de l'écosystème en direct · inspecteur de nœud · assistant IA intégré — <a href="https://monitor.modelmarket.dev/"><b>ouvrir la démo →</b></a></sub>
</p>
...
### 🔎 Scanner de chaîne (SCAN)

Un petit explorateur de blocs pour **la chaîne propre à ce royaume**. La bulle UNI fait tourner son
propre Anvil et, avant cela, le monitor n'avait jamais lu le moindre bloc. Il s'ouvre avec le bouton
**SCAN** de la barre de contrôle.

| Endpoint | Ce qu'il répond |
|----------|-----------------|
| `GET /api/chain/scan?limit=40` | transactions récentes, les plus récentes d'abord |
| `GET /api/chain/tx/{hash}` | une transaction avec son reçu (statut / gas / logs) |
| `GET /api/chain/address/{addr}` | solde, nonce, taille du code, transactions récentes |

- **Lié au royaume** — il lit l'`ALIEN_EVM_RPC` de son propre processus : le monitor UNI scanne la
  bulle, le monitor LIVE sa propre chaîne.
- **Auth de lecture**, comme toutes les routes de lecture (ouvert si `ALIEN_PUBLIC_READ=1`).
- **Des noms, jamais des suppositions** — les adresses viennent des `config/deployments/*.json` de ce
  royaume et de l'`universe_config.json` de la bulle ; les méthodes d'une table de sélecteurs de
  4 octets construite sur les vrais ABI. Un sélecteur inconnu reste en hex.
- **Alerte d'identité de chaîne** — si le RPC répond avec un chainId différent du réseau configuré,
  le scan et `/api/chain/status` renvoient `chain_mismatch` et le panneau affiche une bannière.
- **Jamais de 500** — toute erreur donne un scan vide avec une `note` ; un scanner ne doit pas
  emporter le tableau de bord.

Son utilité : une adresse avec `code_size: 0` qui reçoit un flux régulier d'appels *réussis*, c'est un
composant qui pilote un contrat qui n'existe plus — un appel vers une adresse sans code ne revert pas.
C'est ainsi que le relayer de la loterie UNI a passé neuf jours à appeler `openRound()` dans le vide.
...
URLs publiques (derrière nginx) : **UNI** https://monitor-uni.modelmarket.dev/ · **LIVE** https://monitor.modelmarket.dev/
