...
面向**本域自有链**的小型区块浏览器。UNI 气泡运行着自己的 Anvil，在此之前监视器从未读过任何一个区块。
通过控制栏的 **SCAN** 按钮打开。

| 接口 | 返回内容 |
|------|----------|
| `GET /api/chain/scan?limit=40` | 最近的交易，最新在前 |
| `GET /api/chain/tx/{hash}` | 单笔交易及其回执（状态 / gas / 日志） |
| `GET /api/chain/address/{addr}` | 余额、nonce、代码大小、最近交易 |

- **按域隔离** —— 读取本进程的 `ALIEN_EVM_RPC`：UNI 监视器扫描气泡，LIVE 监视器扫描自己的链。
- **读权限校验**，与其他只读路由一致（`ALIEN_PUBLIC_READ=1` 时公开）。
- **只标注、不猜测** —— 地址来自本域的 `config/deployments/*.json` 与气泡的 `universe_config.json`；
  方法名来自依据真实 ABI 构建的 4 字节选择器表。未知选择器保持十六进制原样。
- **链身份告警** —— 当 RPC 应答的 chainId 与配置网络不一致时，扫描与 `/api/chain/status` 会返回
  `chain_mismatch`，面板显示提示条。
- **绝不返回 500** —— 任何失败都是带 `note` 的空扫描；扫描器不能拖垮仪表盘。

它的价值：一个 `code_size: 0` 的地址持续收到*成功*调用，说明某个组件正在驱动一份已不存在的合约 ——
向无代码地址发起的调用并不会 revert。UNI 抽奖中继正是这样对着空气调用了九天 `openRound()`。
